...
*22 September, 11:55Z.* The measurement above is the whole of this file; no
batch has been imported on this branch yet. The next fire takes **batch 1**
from the cell that trails most — Europe's 16th century, or the Americas'
16th — and, because every candidate cell is a century with no umbrella over
it, its first act is the umbrella (A6): a period Wikipedia has an article
for, with a span and a region, imported as a record and used to file the
main events already here. **An umbrella that absorbs nothing raises the main
count by one and has to say why (A6); an umbrella chosen so that the events
already here fall inside its span and its subject lowers it**, which is the
cheapest fall this lane will ever get and is where batch 1 should start.
