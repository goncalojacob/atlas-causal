...
src/state.js               { from, to, view, focus, degree, tops, selected, edge, source, place, actor, office, chain, horizon, layers, narrative, step, bbox } ⇄ URL; `edge` is the link the reader has opened — an id of the edge vocabulary and never a slug, ahead of every other card in the panel's precedence and left behind by any of them, and read by no lens; `degree` and `tops` are what the graph draws and nothing else reads them; a null bound is "as far as the data goes", and `layers` is the three names, the base map's six (`relief`, `rivers`, `lakes`, `physical`, `mountains`, `cities`, and not `coast`), and a category of events as `events:<id>` beside them. `DEFAULT_LAYERS` is `LAYERS` without `relief` — the one layer off until a reader asks for it — and is what writes no `?layers=` at all. A link naming a subset means "these and nothing else", so an old `?layers=territories,events` opens with the base map off
