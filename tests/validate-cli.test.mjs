...
test('validate.mjs passes on the fixtures and prints the warnings', () => {
  ...
  assert.match(r.out, /47 records, 3 regions: 0 error\(s\), 3 warning\(s\)/);
});
...
test('build-index.mjs writes an index that validate.mjs --index accepts', async () => {
  ...
  try {
    ...
    assert.match(b.out, /12 events, 10 edges, 4 actors, 3 relations, 11 places, 3 presences, 4 sources/);
