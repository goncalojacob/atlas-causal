...
node tools/import/naturalearth.mjs --source vendor/natural-earth/10m --check --budget   # the base map and the relief bands: data/geo/land-present.json and data/geo/base/; offline, rarely. --check because the elevation grid is a derivation nothing here can redo
...
data/geo/base/<layer>/<cell>.json  the base map at the near level, generated by tools/import/naturalearth.mjs: one file per non-empty cell of the fixed 60x45 degree grid, keys x0y0..x5y3, fetched when the viewport enters the cell. `coast` is lines, so no cut ring is ever stroked as a ring; `relief` is the five elevation bands, fill and no stroke, drawn under everything else the map draws
...
src/demo.js                what the published site says about its own state of repair and what it does not: one flag, off unless `?review=1` asks for it, that the masthead's `N of N read`, a card's `Unread`, a citation's `unchecked` tag and the Discuss/Edit links are behind (M82, A2), and `bylineOf` — the names an account is published under, the record's own with the flag on and the atlas's own without it. Nothing under `data/` changes and nothing here is a filter; `review.html` and the validator are untouched
src/summary.js             pure: what of an imported summary is the source's own account and what is the importer's note about the record's standing. The quoted lead, the credit (the article and the revision it was read at, or the Wikidata item), and the sentences that say nobody has read the record and where `review.html` is — detected by the importer's own markers and never by length, with a catch-all for any sentence still naming the maintainer's page. **Nothing in the record changes**: the card is what changes (M86 §1)
src/standing.js            how far a record has been read, said in one line and counted in one place: `hasBeenRead` over a record's `review` block or over the `reviewed` column the index carries, the sentence a card prints and the `N of N read` the masthead prints. Honesty on the surface and never a filter — nothing here decides what is drawn, and `emphasis.js` does not import it
...
src/vocab.js               the closed vocabularies: a leaf module with the edge types, the relation types, the lens kinds and the four place precisions, and the id patterns built from them. `PRECISIONS` says which of the four are **coarse** — a region and a country, which are areas and not points on the ground — and the map draws those wider and fainter; `state.js` and `contribute/bundle.js` import it rather than copying it
src/confidence.js          how sure the atlas is, and how sure it looks: a leaf module with the three confidences in order — `graph.js` and `validate/rules.js` import it rather than copying it — and the class each is drawn with. Every line that stands for an edge reads it, the graph's and the map's walk alike, so two pictures of one record cannot disagree about which link is the shaky one. It hands back a class and never a number: type owns the dash pattern and the weight, confidence takes how solidly the line is inked, and `src/style.css` is where that is spent (M73)
...
src/large.js               pure: which events are large — `scope` written by a person, or parts falling in more than one region lane, judged against the region lanes always. The timeline's band, the map's wash and its corner line all read this one answer
src/parts.js               pure: being part of something, both ways round. `parentsOf(event)` is the one place any of `parent`'s three shapes is read — absent or null, one id, a list of ids (M79) — and the validator, the index, the three views, the card, the form and `m42-pool` all come through it; `isParent` is whether an event has parts, an active record `childrenOf` lists at least one active child for. The ring the map, the timeline and the graph draw around a parent's mark reads that one answer, at every zoom and under every grouping
...
src/map-band.js            the band on the map, always (M64, M75): the slim strip over the map pane's top edge, built with the map and present on every visit — the owner, 21 September, *"The dates two-handled band should not be hidden"*, so M64's toggle and the preference behind it are gone and there is nothing to press and nothing to remember. An overlay and not a row of the grid, so the map pane is still the layout's own height (M60)
src/window-control.js      what the map is looking at, in the masthead, on every view: the count with the pin that gives the world back, and the standing line beside it. Since M80 the count says **what** it counts — "N main events of M in view" at rest, where the picture is the main events (M65) and M is the active corpus, and "N of M events in view" under a lens, where what the lens kept is not a set of main events. `viewCountText` is that one sentence, pure
src/graph-filters.js       the graph's own control, beside the map's layer switches and shown where those are hidden: the degree floor (how many active links an event needs to be drawn). Writes `degree`; `graph-view/arrangement.js` is where it is applied, outside a lens and never inside one. "Top level only" stood beside it until M83 (B10), when it was removed: the resting picture has been the top level everywhere since M65
src/view-key.js            the key to the marks on a picture, and the same box on the map and the timeline that the graph has had since M28 (M82, A7): `MAP_ROWS` and `TIMELINE_ROWS` are the shapes a reader meets and what each means, as data, and the box folds to one button on a phone. Inked by `.view-key` rules that repeat the picture's own declarations and tokens rather than its selectors — `.map .mark` is how a test asks whether the map has drawn — and a browser test holds a row and a real mark to the same computed ink
src/lens-chips.js          the lens in the masthead: one chip per focus with its own ×, "all of these" and "show everything". The back half of the grouping picker M77 removed
src/lanes.js               the one file that decides what a lane is: since M77 the packing and nothing else — as many unlabelled rows as it takes for no two bars and no two titles to overlap — and where a bar goes in one. A lane is an id and its members since M83 (B11); the `label` and `other` fields went with the 120 px gutter the timeline kept for a name M77 had taken away
src/panes.js               the one edge a reader can drag — how wide the panel is — remembered per reader and never in the URL. The timeline's edge went with the strip (M60) and the map band's toggle with the button (M75); a size or a word stored by a version that had either is read into nothing
...
src/state.js               { from, to, view, focus, degree, selected, edge, source, place, actor, office, chain, horizon, layers, narrative, step, bbox } ⇄ URL; `edge` is the link the reader has opened — an id of the edge vocabulary and never a slug, ahead of every other card in the panel's precedence and left behind by any of them, and read by no lens; `degree` is what the graph draws and nothing else reads it; a null bound is "as far as the data goes", and `layers` is the three names, the base map's six (`relief`, `rivers`, `lakes`, `physical`, `mountains`, `cities`, and not `coast`), and a category of events as `events:<id>` beside them. `DEFAULT_LAYERS` is `LAYERS` without `relief` — the one layer off until a reader asks for it — and is what writes no `?layers=` at all. A link naming a subset means "these and nothing else", so an old `?layers=territories,events` opens with the base map off
...
src/graph-view/stretch.js  pure: how much wider than the arrangement time is drawn when the reader zooms in, and how much wider it may ever be. `k` is the zoom every mark and every label divides its size by; `s` is the stretch, and what the reader sees magnified is `k * s` across and `k` down. The cap is four, because what a stretch can part it parts in the first two doublings and what is left at four times is what falls in the same year, which no stretch parts at all (M81)
...
src/graph-view/label-fit.js  pure: the arithmetic of a graph label — how wide a name is at this zoom, where the text starts, and the slice of the picture a label took at the world view
...
src/panel/summary.js       a record's summary as a card prints it: the source's own words, one credit line linking the revision or the item, and the importer's note only under `?review=1` (src/demo.js). One function and the five cards that print prose — event, actor, place, office, and a narrative's step — so the paragraph the review found in two thirds of the bodies cannot come back in one of them
...
src/compose/narrative.js   pure: the draft a reader is building a narrative out of — the events picked as steps, in the order picked, the text beside each, the window computed from them and never typed — and the three translations out of it: the record, through the contribution form's own `buildRecord`; the verdict, through the validator's own `validate()`; the GitHub new-issue URL, through `submit.js`. The draft lives in localStorage the way `panes.js`'s sizes do and never in the URL
src/compose/composer.js    the composer itself (M71): the fourth column of the atlas, opened from the masthead. A click on an event is a step, because the click that chose it is M65's own; a step is always an event the atlas has. **Imported dynamically, the first time it is opened** — it brings the schemas, the rules and the record builder with it, which is what first paint would otherwise pay. Submitting opens the reader's own browser on a prefilled issue: no token, no secret, no API call, no request at all
...
tools/m53-retake.mjs       the coverage row of `docs/m53-polities.md` §4.1, measured and written: how many active events name an actor alive in the year they start, over the two M50 chains and over the corpus, with the active count the row is filed under. `--write` puts it in the document, so a records lane retakes the count with a command rather than a person retyping four numbers, and `tests/m53.test.mjs` reads a row with the same functions
...
tools/import/elevation.mjs  pure: the committed elevation grid -> the five bands. `vendor/elevation/etopo5-10min.i2` read as node registration — the value at (row, column) is the height at 90 - row/6 by column/6, which is the reading that reproduces `vendor/README.md`'s own spot checks — and marching squares over it at the five frozen edges of `BAND_EDGES` (0, 200, 500, 1000, 2000 m; below sea level is not a band). A band is its own edge's rings plus the next edge's, drawn even-odd, so the ground above it is a hole
...
vendor/                    INPUTS: the sources the geometry imports run on, gzipped, never data, never served, never in deploy.yml's allowlist. `vendor/elevation/etopo5-10min.i2` is the one that is not a download but a derivation — ETOPO5 averaged 2 x 2 — and the run does not download it
