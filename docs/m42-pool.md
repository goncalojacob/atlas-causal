...
*The fourth curation fire, 02:06Z. A11(a) over every active event and A13's
relations pass beside it; nothing imported. All six A12 passes and all six of
A14's carry their sections in this file already, so none was re-run.*

| | before | after |
| --- | --- | --- |
| corpus | 993 active | **993 active** |
| **main** | 242 | **238** |
| filed | 751 | **755** |
| active edges | 933 | **938** |
| largest connected component | 654 | **656** |
| second component | 13 | 13, the Korean War's battles, unmoved |
| components | 233 | **229** |
| events with no edge at all | 177 | **172** |
| events with no place | 54 | **44** |
| events naming no actor | 595 | **595** |
| events opening on the import's placeholder summary | 14 | **14** |
| validator | 0 errors, 481 warnings | **0 errors, 474 warnings** |

**Per lane (A10), unchanged in every active cell** because nothing was
imported: active Europe 426, Americas 236, Africa 166, Asia 165 — so A10's
order of need still puts **Asia first by one record**, exactly as the stand
after batch 54 left it. Main by lane moved, because the filings did: Europe
89 → 88, Asia 67 → 64, Americas 55, Africa 31.

### The network, which decided what this fire could do

**Wikidata answered and Wikipedia mostly did not.** `Special:EntityData` — the
endpoint the 24 September fire found round the query service's lag — took
every item read here without complaint. The action API answered **429 to
roughly two of every three requests** for the whole fire; a plain `curl` with
the import's own user agent was refused twice and served once. Nothing was
disabled and no limit was raised: the fetcher used here retries a 429 after
1.2 seconds and backs off to twenty, one request in flight, and on those terms
it read **378 whole articles out of 395 asked for** in about four minutes. A
fire that meets the same wall should back off rather than conclude the API is
down — and should not conclude from one 429 that the network is gone, which is
what a single probe would have said here. That is **deviation 1440**.

### Summaries — fourteen left and nothing a fire can do about them

14 active events open on the import's placeholder. **None was written and none
could be**: every one of the fourteen was re-read against its Wikidata item
this fire, and **not one has an English sitelink at all**.

| | |
| --- | --- |
| written from the article's lead | **0** |
| left: no English article at all | **14** |

Thirteen carry no `wikipedia` key. The fourteenth, `milk-bar-cafe-bombing`,
carries one — and the 24 September fire already wrote down why: the item's
English sitelink is a redirect to the biography "Zohra Drif", so there is no
article about the bombing to quote. Reading it again this fire confirms it:
what comes back is a biography's lead beginning "Zohra Drif Bitat … born
28 December 1934". The twelve of 24 September have become fourteen only
because two imported records since then are in the same position. **This
count will not fall until somebody writes English articles, and no fire
should keep re-reading it**; a check of the items is cheap and is worth one
run a day, no more.

### Places — A9's chain, ten placed, and the guard that fired five times

54 active events had no place. **Ten are placed, eight place records are new,
two were reused, forty-four are refused.**

| | |
| --- | --- |
| placed, reusing a record already here | 2 (`saint-domingue-q861551`, twice) |
| placed, on a record this pass wrote | 8 |
| at `country` precision | 4 |
| at `region` precision | 6 |
| refused | 44 |

The ten: `battle-of-vilcaconga` → `inca-empire-q28573`,
`blockade-of-saint-domingue` → `saint-domingue-q861551`,
`may-1958-crisis-in-france` → `french-fourth-republic`,
`mexican-war-of-independence` → `northern-america-and-mexico`, `new-laws` →
`spanish-empire`, `saint-domingue-expedition` → `saint-domingue-q861551`,
`spanish-american-wars-of-independence` → `hispanic-america`,
`spanish-conquest-of-the-aztec-empire` → `aztec-empire-q2608489`,
`war-of-jenkins-ear` → `caribbean`, `war-of-the-south` →
`saint-domingue-q861551`.

**Five classes were added to `data/imports/wikidata-seeds.json` → classes**,
because the chain reached them and the table had not decided about them, so
the pass could give the place no precision at all: Q133156 colony, Q3502482
cultural region, Q137186904 transcontinental region, Q33837 archipelago,
Q855697 subcontinent, each `region`, each with the class item's own gloss read
over the network and the place that carried it named in the note. That is the
sanctioned way — a precision is data somebody can argue with — and it is what
turned six of the ten placements from refusals into readings.

**The refusals, in four kinds:**

- **Thirty-two for no coordinate anywhere on the chain**, the same shape as
  the 24 September fire's largest class: the item names no location and has no
  point of its own, or the located item it reaches has no `P625`.
- **Five refused by the lane guard** (deviation 1402), and each is a reading
  worth keeping: `1948-arab-israeli-war` → Q36755 the Sinai Peninsula, which
  is in the Africa lane against a record in Asia; `great-depression` → `P17`
  Q889 **Afghanistan**, which is the country the item happens to name and
  which A14(2) has already said stands for nothing — *the Great Depression
  stands in no single country*; `yom-kippur-war` → Q48214 the Near East, the
  same place A14(2) cleared off this record five weeks of fires ago;
  `execution-of-the-romanov-family` → Q1381934 the Ipatiev House, and
  `ilinden-preobrazhenie-uprising` → Q2121602 the Monastir vilayet. **The last
  two are the guard refusing a correct place because a record's own lane is
  wrong**: the Ipatiev House is in Yekaterinburg, which the lane polygons put
  in Europe and the record calls Asia — deviation 1423's open question, *should
  the europe lane hold Russian Asia?* — and the Ilinden uprising was fought in
  Macedonia and Thrace, which is Europe, against a record filed in Asia because
  its polity was the Ottoman Empire. Neither is the place's fault and a fire
  cannot tell the two cases apart mechanically; both want a person.
- **Two whose chosen point reaches no lane polygon**: `dutch-portuguese-war` →
  Q97 the **Atlantic Ocean**, and `india-pakistan-war-of-1971` → Q2399139 the
  Bangladesh–India border. Nothing may leave the index unable to derive a lane,
  and an ocean is not where a war happened.
- **Five refused because the item's own point is the event's own record.**
  This is **deviation 1441**, and it is one test doing the work of the two the
  24 September fire needed. That fire refused `peruvian-war-of-independence`
  because *a war's span is not one engagement's*; the same five items fail a
  simpler question first — **is the thing carrying the coordinate a place at
  all?** Q665554 is the *Japanese Instrument of Surrender*, a document whose
  `P625` is Tokyo Bay; taking it would have written a place record named
  "Japanese Instrument of Surrender" and asserted a place by that name that
  nobody has ever called anything. So a self-`P625` is refused where the class
  table says the item is an **event**, which catches the document and the four
  conquests (`peruvian-war-of-independence`,
  `spanish-conquest-of-guatemala`, `spanish-conquest-of-iberian-navarre`,
  `spanish-conquest-of-the-inca-empire`) in one rule. The span test is not
  needed and is not kept.

### A7 — nine intervals widened, and the edge that then contradicted itself

Every active event with a lead on disk or an article read this fire — 993 of
them — had its span compared against the year range in its cached lead's first
sentence. **54 disagreed and nine are real**; the other 45 are the containing
war's years, a UN resolution number read as a year (`2011-military-intervention-in-libya`
and `operation-unified-protector` both name Resolutions 1970 and 1973), or a
date belonging to somebody in the sentence rather than to the record.

| record | was | is | read from its own first sentence |
| --- | --- | --- | --- |
| `1999-2002-farc-government-peace-process` | 1999 | 1999-01-07 to 2002-02-20 | "from January 7, 1999, to February 20, 2002" |
| `ecuadorian-war-of-independence` | 1820–1823 | 1809–1822 | "was fought from 1809 to 1822" |
| `peruvian-war-of-independence` | 1821–1824 | 1809–1826 | "a series of military conflicts in Peru from 1809 to 1826" |
| `eritrean-ethiopian-war` | 2000 | 1998-05-06 to 2000-06-18 | "took place from May 6, 1998 to June 18, 2000" |
| `franco-russian-alliance` | 1894-01-04 | 1891–1917 | "formed by the agreements of 1891–94; it lasted until 1917" |
| `bretton-woods-system` | 1944-07-22 | 1944–1976 | "after the 1944 Bretton Woods Agreement until the Jamaica Accords in 1976" |
| `revolt-of-czechoslovak-legion` | 1920-09 | 1918-05 to 1920 | "beginning in May 1918 and persisting through evacuation … in 1920" |
| `war-of-the-sixth-coalition` | 1813–1814 | 1812-12 to 1814-05 | "(December 1812 – May 1814)" |
| `world-war-ii-in-albania` | 1942–1944 | 1939–1944 | "In Albania, World War II began with its invasion by Italy in April 1939." |

**`eritrean-ethiopian-war` was the worst of the nine and nothing had caught
it**: the record carried `date` 2000-06-18 against an `endDate` of
2000-05-25, so it began three weeks after it ended. Rule 15 compares years and
both were 2000.

**One widening turned an existing edge into a contradiction, and A13's own
instrument is what it got.** `world-war-ii --caused--> world-war-ii-in-albania`
quotes, in its own explanation, the sentence this pass read the new date from.
April 1939 is five months before the 1 September this atlas dates the war
itself from, so the war cannot have caused the theatre named after it. The
edge is now **`disputed`**, with the article at its revision in the dispute and
`a13-contradicted` in its flags. **The April is not written on the record**:
`when` is 1939–1944 with no month, because a month there makes rule 4 read a
containment as an impossible cause and error on it, and what is plainly true of
the pair — that one is part of the other — `parent` already says. The date is
not kept wrong and the edge is not turned round silently, which is what A14(1)
and A13 each ask for. That is **deviation 1442**.

One candidate was left as it stands and is worth naming: `south-lebanon-conflict`
is dated 1985–2000 against a first sentence reading "from 1982 or 1985 until
Israel's withdrawal in 2000". **The article itself does not decide**, and A7
widens from what the article states.

### A13 — the relations pass

**395 events were the target set**: the 153 created since the last curation
fire and the 339 that sit outside the largest connected component. **378
articles were read whole** at their current revision; 17 refused, every one
for having no English article to read. Matching used the 23 September fire's
corrected matcher, unchanged.

| | |
| --- | --- |
| articles read whole | **378** |
| pairs where a causal cue and another event's name share a sentence | **211** |
| pairs the atlas already holds an edge for | 70 |
| fresh pairs to read | **141** |
| refused before reading, for touching an umbrella at either end | 36 |
| refused before reading, for running between a record and its own ancestor (C8) | 67 |
| **read one by one** | **56** |
| **edges written** | **5** |
| edges disputed | **1** (the A7 pair above) |
| largest component | 654 → **656** |

**The five, each quoting the sentence that carries the claim:**

| edge | type | read at |
| --- | --- | --- |
| `sinking-of-ara-general-belgrano` → `battle-of-san-carlos` | precondition-of | the battle's § Argentine forces |
| `insurrection-of-10-august-1792` → `execution-of-louis-xvi` | precondition-of | the execution's § Background |
| `tet-offensive` → `my-lai-massacre` | precondition-of | the massacre's § Background |
| `war-of-the-fourth-coalition` → `peninsular-war` | precondition-of | the coalition war's § Results |
| `storming-of-the-bastille` → `women-s-march-on-versailles` | precondition-of | the march's § Royal banquet |

All five are `precondition-of` and all five `probable`, which is what A2 allows
through Wikipedia. The Belgrano one is the fire's best and the one a reader
would miss: the atlas held both records under `falklands-war` and nothing
between them, and the battle's own article says why the landings were opposed
from the air alone — *"the Navy's surface fleet remaining in port after the
sinking of ARA General Belgrano"*.

### The finding, which is bigger than any edge in it

**`reacted-to` reads backwards on the card, on all 53 of them, and the data is
not what is wrong.** Deviation 1420 settled the data side a week ago — *`A
--reacted-to--> B` says B answered A*, read off rule 4 and not off the English
verb — and this fire settles what that means for the picture. `A
--reacted-to--> B` is *drawn* as "A reacted to B"; rule 4 requires `from` not
to start after `to`; and a reaction is by definition the later of the two. **The two cannot both hold**, and every `reacted-to` record
in this corpus was written the only way that validates — `from` is the thing
reacted *to* and `to` is the reaction. `battle-of-liaoyang --reacted-to-->
battle-of-shaho` means Shaho answered Liaoyang, which is the history and what
its own explanation argues; the card says Liaoyang answered Shaho, which is
impossible.

This matters beyond wording, because **the 24 September fire disputed two
edges on the card's reading**: `libyan-civil-war --reacted-to-->
2011-military-intervention-in-libya` and `sierra-leone-civil-war
--reacted-to--> second-liberian-civil-war`. Under the corpus's own convention
the first says the intervention answered the war, which is exactly what that
fire quoted the article as saying. This fire did not turn either round — a
dispute is a reviewer's to lift — and it added no dispute of its own on this
ground, having checked all three `reacted-to` pairs its matcher reached
(`battle-of-liaoyang`/`battle-of-shaho`, `battle-of-nanshan`/`battle-of-te-li-ssu`,
`battle-of-the-yellow-sea`/`battle-off-ulsan`) and found each one right under
the convention and wrong under the label. **The fix is one word in
`src/vocab.js` or one arrow in the card, it is a display change, and it is
lane A's and not this run's.** That is **deviation 1443**, and it is the
largest thing this fire found.

### The refusals, which are the load-bearing half

**103 of the 141 fresh pairs were refused before anybody read them**, by two
standing rules: 36 for touching one of the 18 umbrella records at either end
(batch 31a's rule) and 67 for running between a record and its own ancestor,
which is `parent`'s job and which C8 leaves standing until the owner decides.
The umbrella cost is the same as ever and still falls on the links that would
join continents: `decolonisation-of-africa` → `world-war-i` and `→
world-war-ii` from its own § External causes, `scramble-for-africa` →
`world-war-i` from § Aftermath, `arab-spring` → `iraq-war`, `interwar-period`
→ `second-italo-ethiopian-war` and → `munich-agreement` and → `march-on-rome`.

**Of the 56 read one by one, 51 were refused**, in six kinds:

- **Twenty-two are chronology with no claim in them** — "After", "Following",
  "shortly after" and nothing else: `battle-of-seal-cove` and Goose Green,
  `japanese-invasion-of-sakhalin` and Tsushima, `capture-of-ayaviri` and
  Sangarará, `spanish-conquest-of-guatemala` and the Aztec Empire. This is
  A14(4)'s own refusal class applied at the point of writing rather than
  afterwards.
- **Fourteen because the sentence's subject is not either record.** The
  clearest is `french-invasion-of-russia`'s § Background — *"He secured
  victories in the War of the Third Coalition (1803–1806, leading to the
  dissolution of the thousand-year-old Holy Roman Empire)"* — whose stated
  effect is the Empire's dissolution and not the invasion of Russia;
  `battle-of-wireless-ridge` offered Goose Green from a sentence whose effect
  is a change of command, and `reign-of-terror` offered the execution of Louis
  XVI from a sentence about the First Coalition.
- **Six are enumerations**, the shape the 24 September fire refused twice and
  which returns here five times over: `insurrection-of-31-may-2-june-1793`
  standing third in a list of three great insurrections, `tennis-court-oath`
  "preceded the Storming of the Bastille in July, the abolition of feudalism in
  August", `decolonisation-of-africa`'s lead listing its own major events.
- **Four are comparisons** — `black-monday` "sparked fears of … a reprise of
  the Great Depression", `war-in-darfur` "led to comparisons with the Rwandan
  genocide", `battle-of-khe-sanh` and Dien Bien Phu, `eritrean-civil-wars` and
  a perception of the Arab–Israeli conflict. Each was refused by an earlier
  fire on the same words.
- **Three on the arrow of time**, of which `peruvian-war-of-independence` →
  `battle-of-rancagua` is the shape: a war widened this very fire to start in
  1809 cannot be the cause of a battle of 1814 that the article places inside
  it.
- **Two namesakes.** `battle-of-bailen` reaches `war-of-attrition`, the 1967–70
  one, out of *"a long war of attrition waged against determined Spanish
  guerrillas"*; `fourth-battle-of-gao` reaches `battle-of-gao` out of a
  sentence about the **Third** Battle of Gao. The matcher's eight-character
  floor does not catch a phrase that is also a common noun, and neither would a
  longer one.

### P710 — 348 eligible, nothing written, and the reason is new

595 active events name no actor; **348 carry a category the vocabulary has a
role for** — `war` is a `belligerent`, `treaty` a `signatory` — and every one
of them was read against its item.

| | |
| --- | --- |
| the item names no participant | **176** |
| participants the atlas holds none of | **139** |
| a partial list, withheld under deviation 1404 | **32** |
| a whole list, and still refused | **1** |
| **written** | **0** |

**The one whole list is the finding.** `battle-of-crete-a-pierrot`, of 1802,
names Q142 and Q790 as participants and the atlas holds both — as `france` and
`haiti`, two CShapes polity records that **both begin in 1886**, eighty-four
years after the battle. Writing the line would have named two polities that did
not exist when it was fought, and the identity match that found them is on the
Wikidata item and says nothing about the period. **So a participant is written
only where the actor record the atlas holds spans the event's own date**, which
is **deviation 1444** and the second gate deviation 1404 needed: 1404 asks
whether the list is whole, this asks whether the actors are the right ones. It
will bite most often exactly where A9's `country` placements are landing, on
the pre-1886 corpus, because CShapes is where those actors come from.

### Parents (A6, A8) — four filed, and the lane test still says nothing

The 18 umbrellas were measured against the 242 main events. The span-and-lane
filter returns 118 candidates and, for a **polity** umbrella, nearly all of
them are nonsense for the reason the 24 September fire wrote down — the Estado
Novo would swallow the Molotov–Ribbentrop pact and the Afghan conflict would
swallow nineteen events across Asia, because the lane says only which continent.
Taken by the corrected test — **the lane *or* an actor the umbrella itself
names** — thirteen candidates remain and **four were filed**:

| filed | under | why |
| --- | --- | --- |
| `aberdeen-act-1845` | `empire-of-brazil-1822-1889` | inside the span, and the Empire is the act's target, one of its two actors |
| `goa-annexed-1961` | `estado-novo-1933-1974` | inside the span, and the Estado Novo is the government whose territory was taken |
| `east-timor-invasion-1975` | `third-portuguese-republic-since-1974` | inside the span, and the republic is named as the party that lost the territory |
| `macau-handover-1999` | `third-portuguese-republic-since-1974` | inside the span, and the republic is the record's only actor |

**Nine were refused and eight of them by M67's rule 1** — a period named for a
form of government does not contain the act that created or destroyed it:
`1964-brazilian-coup-detat`, `operation-brother-sam-1964` and
`the-base-reforms-rally-1964` against the Brazilian military dictatorship;
`proclamation-of-the-brazilian-republic-1889` against the Empire of Brazil;
`republic-proclaimed-1910` against the First Portuguese Republic; and
`carnation-revolution-1974` against all three of the Estado Novo, the
Portuguese Colonial War and the Third Republic, which it ended, ended and began
respectively. The ninth is `molotov-ribbentrop-pact` under `world-war-ii`,
signed nine days before the war it helped bring about; `charter-of-the-united-nations`
is refused beside it because a war is its own umbrella **by its fighting**
(A6) and the Charter was signed in San Francisco in June 1945.

`goa-annexed-1961` was filed under the Estado Novo and **not** under
`portuguese-colonial-war-1961-1974`, although the span and three actors fit:
the war that record names is the fighting in Africa, and the annexation of Goa
was not part of it.

**The main count fell from 242 to 238** and has not risen through twenty-five
batches, four curation fires and A14.

### Polities' descriptions — nothing to write, and the fifteen are the same fifteen

115 polities are named by an active event and **100 carry a description**. The
15 that do not are **exactly the 15 A14(5) refused**, each with its reason
written there — `european-economic-community`, `inca-empire`, `dutch-republic`,
`viceroyalty-of-brazil`, `orange-free-state` and
`vietnam-democratic-republic-of` on the dissolution-year gate;
`united-kingdom-before-1886`, `castille`, `persia` and `transvaal` because
neither signal could be checked; `military-dictatorship` because its three
names resolve to three items; and `england-and-ireland`, `algeria-under-france`,
`cuba-under-spain` and `vietnam-annam-cochin-china-tonkin` because no English
article their names resolve to. **No polity newly named since 24 September
lacks a description**, so the clause is met and this fire wrote none. A fire
should measure this and move on until an import names a new polity.

### One drift A14(3) did not hold

`declaration-by-united-nations` cites `wikipedia-en` at revision 1376063232 and
the cache under `tools/import/cache/wikipedia/` holds 1369044189. A14(3) cached
every citation at the revision it cites; this one has drifted since, or was
cached before its citation was rewritten. It is one record out of the whole
corpus and it is why this fire did **not** widen that record's span, which its
first sentence would otherwise have supported ("signed by 47 national
governments between 1942 and 1945" against a record of 1942 alone): A7 reads
the article **at the revision the record cites**, and this fire could not.
Worth a re-cache by whichever fire next runs A14(3)'s pass.

## Where the run stands after the curation fire of 25 September, for the fire that picks it up

*25 September, 02:06Z onward. A curation fire: nothing imported, no code
changed, one data file outside `data/events`, `data/edges` and `data/places`
touched — `data/imports/wikidata-seeds.json`, five classes added so A9's chain
could say how coarse a place is.*

| | |
| --- | --- |
| corpus | **993 active**, unmoved |
| **main** | **238** (−4), and it has not risen in twenty-five batches and four curation fires |
| filed | 755 (+4) |
| active edges | **938** (+5) |
| largest connected component | **656** (+2) |
| second component | **13**, the Korean War's battles, unmoved |
| components | **229** (−4) |
| events with no edge at all | **172** (−5) |
| events with no place | **44** (−10) |
| validator | **0 errors, 474 warnings** (−7) |
| per lane, active | Europe 426, Americas 236, **Africa 166**, **Asia 165** |
| per lane, main | Europe 88, Asia 64, Americas 55, Africa 31 |

**A10's order of need is unchanged**: Asia trails at 165 against Africa's 166,
so the next import batch is Asia's unless a chain crosses out of it, and the
stand after batch 54 names the row — `Q2035177`, the Battle of Bloody Ridge,
with `Q489561`, Heartbreak Ridge, beside it, one stated sentence apart.

**What is open, in the order a fire should weigh it:**

- **`reacted-to` reads backwards on the card, on all 53 of them** (deviation
  1443, above). It is a display change, it is one word, and until it is made
  every reader of an edge card is told the reverse of what the record says.
  The two disputes the 24 September fire wrote on this ground are still open
  and a reviewer, not a fire, should lift them.
- **C8 is still the largest thing standing between this atlas and the chains
  the owner asked for**, and this fire measured the cost again from the other
  side: **67 of the 141 fresh pairs A13 found were refused for running between
  a record and its own ancestor** — nearly half the pass — against 36 for the
  umbrella rule. The standing measurement is batch 53's: thirteen records, a
  whole war, held outside the largest component by one rule, and one edge
  `korean-war` → `first-battle-of-seoul` would join all thirteen.
- **The next Africa batch should still look for the Ménaka offensive of
  2022–2023 first** (batch 54's stand), and `Q21028650`, the Algiers Accords of
  2015, is still refused on its class and still a question for a person.
- **Two records' own lanes are wrong and the place guard is what found them**:
  ...
  `region` — no fire has, because the guard refuses the place rather than
  correcting the record, and which of the two is at fault is not mechanical.
- **`declaration-by-united-nations`'s cache has drifted from its citation** and
  wants a re-cache under A14(3)'s pass; its span is the one A7 widening this
  fire could not make.
- **`JNIM`, the Wagner Group and the CSP are still not actors here**
  (`Q28877411`, `Q36597284`, `Q111207670`), and A12(4) as written maps only
  participants the atlas already holds. Deviation 1444 now adds a second reason
  a participant is refused — the actor record must span the event's date — and
  the two together are why P710 has written nothing in three fires.
- **`battle-of-nam-river` is still filed one day away from its own umbrella**
  and should be re-filed under `battle-of-the-pusan-perimeter` by whichever
  fire first widens that record's interval under A7 (deviation 1437). This
  fire's A7 pass did not reach it: the Pusan Perimeter's first sentence states
  no span.
- **`the-persecution-of-the-jews-1933-1941` and `porajmos` are still a two-node
  component.**
- **Two of the fifteen tombstones still want an A6 umbrella and not an edge**:
...
- **The fourteen placeholder summaries are not a fire's to fix** and should be
  checked once a day and left, as above.
- **The next curation fire is the first fire after 02:00Z on 26 September**,
  ...
  corpus returns the same rows, so it is worth running over what an import adds
  and over what is still outside the largest component — which is what this
...
## Batch 59 — the Gao counterattacks of February and March 2013, and the vein ranked rather than guessed

*25 September, the fire that claimed at 12:08Z. Today already carries a
`## Curation 2026-09-25` section and **all six of A14's passes carry theirs**, so
this is an import fire and every one of the six is skipped by its own rule. The
claim at 09:23Z was 164 minutes old and `origin/m42` had not been pushed for 95
minutes, so the protocol's ninety-minute clause let this fire take it.
...
A10's order of need reads **Africa 174, Asia 174** after the merge, against
Europe's 465 and the Americas' 310 — a tie, as it was at batch 58, where A10's
own order of need (Africa, South America, Asia) gives the batch to Africa.
Deviation 1449 agrees, and this fire made 1449 a measurement instead of a
judgement (**deviation 1450**): every active event inside the largest connected
component that carries a Wikidata item — 544 of them — was handed to the query
service in one `VALUES` block and grouped by `P361`, which ranks **every vein in
the atlas** by exactly the number deviation 1449 says to choose on.

| the vein | held records inside the 673 | rows with an English article |
| --- | --- | --- |
| `Q2946372` Mali War | **22** | 83 |
| `Q8683` Cold War | 21 | — |
| `Q159950` Russo-Japanese War | 15 | — |
| `Q81545` Libyan Civil War | 15 | 33 |
| `Q362` World War II | 13 | — |
| `Q8740` Vietnam War | 10 | 300 |
| `Q243620` Somali Civil War | 5 | 29 |
| `Q1773926` Congo Crisis | 5 | 12 |

**The Mali War is the strongest vein in this atlas and it is in the lane that
trails**, so the two answers are one answer. Batch 58's stand had pointed the
next fire at the Vietnam vein on its 286 free rows; ranked the way 1449 asks, the
Vietnam vein is sixth. **A stand that names a vein by its free rows is naming the
wrong thing, and a query answers it in one request.** The query is a POST and not
a GET: 544 `wd:` terms is 6.5 kB of query text and the service returns 431 on the
URL.

Read through the query service (`query.wikidata.org/sparql`) for the veins and
the rows, `Special:EntityData/<qid>.json` for each item's claims, and the
**action API** (`api.php`, `prop=revisions|extracts`, twelve titles a request,
with a `User-Agent`) for the articles and the leads. The REST
`page/summary/` route returned **429 on twenty consecutive requests** with no
`User-Agent` set; the action API answered every batch at 200. That is deviation
1424's route confirmed a second time and the cheaper way to walk twenty
candidates at once.

A11 (b)'s partition check was made before the import, against `origin/m42b`'s
`data/events/` and `data/places/`: neither record nor either place is there.

In date order:

| the record | sitelinks | dates | placed at | filed under |
| --- | --- | --- | --- | --- |
| `battle-of-imenas` | 5 | 27 Feb – 1 Mar 2013 | `imenas` (new) | `mali-war` |
| `battle-of-tin-keraten` | 4 | 6 Mar 2013 | `tin-keraten` (new) | `mali-war` |

Both pass the three tests unchanged since batch 20 — **depth**, each carrying
`P361` `Q2946372`; **span**, each inside the Mali War's 2012 to the present; and
**lane**, each deriving Africa from its own point. Both filed, so **main stays at
239**, unmoved now through thirty batches, four curation fires and A14.

**One interval was widened under A7 and one was not touched.** `Q6619913` carries
a single `P585` of **1 March 2013** — the last day of the fighting recorded as
though it were the whole of it, which is `Q2395085`'s case from batch 57 — while
the article's infobox at revision 1370265996 reads "27 February – 1 March 2013"
and its background dates the launch of the operation to 27 February in prose.
The **start** is widened back to 27 February; nothing is narrowed, and the record
keeps the `date` flag with the note saying what was changed from what.
`Q6602439`'s single `P585` of 6 March is the one date its article gives.

**No class was added, for an event or for a place.** Both items are `Q178561`,
battle, which has been in the table since batch 50.

### The four edges, and the two records they rescued

Four edges, every one quoted from an article at the revision in its own locator;
the full table with the sentences is in `docs/m42-connections.md` under
"Batch 59".

| from | type | to | confidence |
| --- | --- | --- | --- |
| `second-battle-of-gao` | reacted-to | `third-battle-of-gao` | probable |
| `second-battle-of-gao` | reacted-to | `fourth-battle-of-gao` | probable |
| `fourth-battle-of-gao` | precondition-of | `battle-of-imenas` | probable |
| `battle-of-imenas` | precondition-of | `battle-of-tin-keraten` | probable |

**The largest connected component moves, 673 to 677**, and four is twice the two
records that arrived. Two of the four edges have **no arrival at either end**:
`second-battle-of-gao --reacted-to--> third-battle-of-gao` and the same edge to
the fourth join a two-node component the atlas has carried since batch 56, and
both rest on a sentence in the *third* article — the Imenas one this batch
imported. Neither Gao article names the other; the Imenas background names both,
with the wikilinks to say which records it means. That is batch 57's Khe Sanh
lesson with two rescued instead of one, and it is A5's clause that a batch writes
its edges "to what already exists as well as to itself" doing the heavier half of
the work. **The cheapest thing an import can do is read the article it imported
for the records it did not.**

**`reacted-to` is written twice and the second opens no path.**
`third-battle-of-gao --precondition-of--> fourth-battle-of-gao` has been here
since batch 56, so the fourth reaches the 673 the moment the third does. The
second edge is written because the sentence names two attempts and the second of
them is about that record: a reader on the fourth battle's card should find the
capture it answered there and not one hop away. Both are written in the direction
**deviation 1420** settled — `A --reacted-to--> B` says *B answered A* — so the
capture of Gao stands at `from` and the raid to retake it at `to`.

**The component count falls by one, 309 to 308, and nothing new is stranded.**
Both arrivals carry an edge, so **the edgeless count stays at 232** and the
validator's warning count stays at **586**: this is the first batch of this run
that added two events and no `degree-zero` at all.

### A9's places, and the item that gives two points

**Two place records are new and neither was hand-corrected.** Both carry
`summary: null`, which is what A14 (6) left every imported place at, and the
`a9-place` flag; both derive the Africa lane from their own point.

- **`imenas`** is A9's **first step** as A12 (2) corrects it: `Q6619913`'s own
  `P625` (16.3 N, 0.717 E), `precision: point`. The item *also* carries `P276`
  `Q188904`, Gao — a place this atlas already holds and the one
  `third-battle-of-gao`, `fourth-battle-of-gao` and `fifth-battle-of-gao` all
  stand on — and taking it would have put four records of this vein on one point.
  A12 (2)'s order is what stopped that: the article puts the fighting "east of
  Gao, between Djebok and al-Moustarat", 75 km from the city, and the item says
  so itself. **This is the clearest case yet for reading `P625` before `P276`.**
- **`tin-keraten`** is the same step with nothing to fall back on: `Q6602439`
  carries no `P276` and no `P131`, only its own `P625` (16.417 N, 0.817 E) and
  `P17` Mali. The article calls the ground "the Wadi Tin Keraten area, located
  100 km east of Gao and northeast of Imenas", which is 17 km from the Imenas
  point, so the two battles get two places and not one.

### The actors, and the two items that name none

**Neither item carries a `P710` at all**, so both arrive with `actors: []`, as in
batches 50, 52, 56 and 58. `docs/m53-polities.md` §4.1 is re-taken with the
batch: **401 of 1,125** by the start rule, 402 by overlap — the numerator
unmoved, the denominator up by eighty-one from the merge and two from the batch.

### What was refused

- **Four edges**, listed in `docs/m42-connections.md`. The one worth naming is
  `battle-of-in-khalil` → `operation-serval`: the In Khalil article says MUJAO
  "specifically targeted the MNLA for their part in siding with the French
  intervention", which argues a reaction and names what was answered — but names
  it in plain words with no wikilink, and deciding that "the French intervention"
  is `operation-serval` is an identification a person should make. A row whose
  only edge needs the run to supply the identification is a row to leave.
- **`Q12016335`, the Battle of Djebok**, 4 sitelinks, dated 12–17 March 2013 and
  the next fight of the same column: refused on two counts. Its article argues no
  edge at all — it names Imenas twice and both times as a compass bearing, "north
  of Imenas" — and its item carries **two `P625` values that disagree by twelve
  kilometres** (16.758 N 0.579 E and 16.65 N 0.517 E), which A9's first step has
  no rule for. A fire that wanted it would have to choose between them or fall to
  `P276` `Q5285264`, and neither is a choice an import should make silently.
- **`Q139556342`, the 2026 Mali offensives**, 11 sitelinks and the strongest free
  row the vein has: read in full and left. Its Background names six held records
  and **every sentence that links them opens on "Following"** — A14 (4)'s refusal
  class — and the one consequence it does argue, the Wagner Group replaced by the
  Africa Corps, is not an event this atlas holds. Its one real chain runs through
  `Q137460579`, the Mali fuel blockade ("the blockade of fuel imports crippled the
  Malian state's ability to function"), and that pair is a good two-record batch
  for a later fire **if** it can find the edge into the 673 this one could not:
  the offensives article names `kidal-offensive` and
  `french-military-withdrawal-from-west-africa` only inside `<ref>` tags, which is
  a citation and not prose.
- **`Q135423253` Battle of Anoumalane** and **`Q125256362` Labbezanga attack**,
  both of which name a held record inside the 673 in prose and neither of which
  argues an edge to it: Anoumalane's is a comparison of two defeats by size, and
  Labbezanga's background is the junta's chronology.
- **`Q123058541`, the Siege of Timbuktu**, 3 sitelinks, ongoing since August
  2023: its lead does argue an edge — "The siege began after the withdrawal of
  MINUSMA" — and **MINUSMA's withdrawal is not a record here**. It is the
  strongest single argument in the vein for a row this atlas cannot yet carry, and
  it would have joined neither the 673 nor the two Timbuktu battles, which its
  article never names.
- **`Q6337939`, Operation Panther**, whose English sitelink is a **disambiguation
  page**: five operations of that name across three wars. Nothing to read and
  nothing to cite.

## Where the run stands after batch 59, for the fire that picks it up

*25 September, the fire that claimed at 12:08Z. One import batch, two events,
four edges, two places, no class added, and two leads cached under
`tools/import/cache/wikipedia/` at the revisions the records cite, so A14 (3)'s
invariant holds over what this fire wrote. No code changed.*

| | |
| --- | --- |
| corpus | **1,125 active** (+2 on the batch, +81 from the merge) |
| **main** | **239**, unmoved through thirty batches and four curation fires |
| filed | 886 (+2) |
| active edges | **994** (+4) |
| largest connected component | **677** (+4 on four edges and two arrivals) |
| second component | **13**, the Korean War's earlier battles, unmoved |
| components | **308** (−1: the Gao pair absorbed, nothing stranded) |
| events with no edge at all | **232** (unchanged) |
| validator | **0 errors, 586 warnings** (unchanged) |
| per lane, active | Europe 465, **Asia 174**, **Africa 176**, Americas 310 |
| per lane, main | Europe 88, Asia 64, Americas 56, Africa 31 |

**A10's order of need has turned over: Asia trails at 174** against Africa's 176,
so the next import batch is Asia's unless a chain crosses out of it.

**What is open, in the order a fire should weigh it:**

- **Rank the veins before reading anything** (deviation 1450). One POST to the
  query service, the qids of every active event inside the largest component in a
  `VALUES` block, grouped by `P361`, and deviation 1449's number falls out for
  every vein at once. This fire's table is above and it should be re-taken rather
  than trusted: it moves with every batch. On it, **Asia's best veins are
  `Q159950` the Russo-Japanese War (15 held inside the component) and `Q8740` the
  Vietnam War (10, and 290 free rows)** — the Korean War does not appear in the
  top thirty at all, which is the same thing batches 51, 53 and 55 found the
  expensive way.
- **The Vietnam vein is the Asia batch and batch 57's stand lists the rows**: 286
  free, 271 dated, and **eight** of the fourteen it holds are now inside the 677
  (`tet-offensive`, `my-lai-massacre`, `paris-peace-accords`, `fall-of-saigon`,
  `cambodian-civil-war`, `laotian-civil-war`, `easter-offensive`,
  `operation-linebacker-ii`, `operation-pocket-money`, `battle-of-khe-sanh`). The
  best-connected free rows by prose are `Q4821674` the 1975 spring offensive (16
  sitelinks, which ends in `fall-of-saigon`), `Q2994973` the Cambodian campaign
  (20) and `Q2095768` Operation Lam Son 719 (19).
- **Six held Vietnam records are still singletons whose articles argue edges
  nobody has written** — `battle-of-hue`, `battle-of-long-tan`,
  `battle-of-ap-bac`, `1963-south-vietnamese-coup`, `battle-of-hamburger-hill`,
  and `battle-of-ia-drang` with `gulf-of-tonkin-incident`. **This batch is the
  proof of what that is worth**: two of its four edges were of exactly that shape
  and they were the two that moved the number. A fire can write them without
  importing anything.
- **`fifth-battle-of-gao` was re-read as batch 56's stand asked and the vein
  still offers nothing.** Its article (revision 1370618201) names no earlier
  battle at all: the raid of 23–24 March arrives out of nowhere and the only
  earlier thing it mentions is 160 Guinean soldiers who "arrived in the city three
  days earlier". It stays a singleton and a later fire should not spend the
  reading again.
- **The two Timbuktu battles are still a two-node component** and neither their
  own articles nor the Siege of Timbuktu's name anything else. The Gao pair had a
  third article that knew both; the Timbuktu pair has none found yet.
- **The Mali fuel blockade and the 2026 offensives are a pair a later fire should
  take together** — the chain between them is stated ("the blockade of fuel
  imports crippled the Malian state's ability to function") — **but only once it
  has an edge into the 677**, which this fire looked for and did not find. Two
  records joined to each other and to nothing else is the walled garden deviation
  1449 is about.
- **C8 is still the largest thing standing between this atlas and the chains the
  owner asked for.** Batch 53's thirteen-record Korean War is still outside the
  677; one edge `korean-war` → `first-battle-of-seoul` would join eighteen.
- **`reacted-to` reads backwards on the card, on all 62 of them** (deviation
  1443) — sixty before this batch, which wrote two more; batch 58's stand counted
  54 and the merge with `origin/m0` brought six of M42b's. One word, a display change, and every reader
  of one of those cards is told the reverse of what the record says.
- **Two records' own lanes are wrong**: `execution-of-the-romanov-family` (the
  owner's question, deviation 1423) and `ilinden-preobrazhenie-uprising`, which is
  simply wrong and a fire could fix by clearing the record's `region`.
- **`declaration-by-united-nations`'s cache has drifted from its citation** and
  ...
  **`the-persecution-of-the-jews-1933-1941` with `porajmos`** are all still as
  ...
  umbrella and not an edge**: `1948-palestine-war` and `balkan-wars`. **The
  fourteen placeholder summaries are not a fire's to fix.**
- **`JNIM`, the Wagner Group, the CSP, the FLA and MUJAO are still not actors
  here.** Every article this batch read names MUJAO and none of the four Mali
  batches since 50 has been able to write a single `P710` because of it. It is the
...
*25 September, the fire that claimed at 14:51Z. Today already carries a
`## Curation 2026-09-25` section and **all six of A14's passes carry theirs**, so
this is an import fire and every one of the six is skipped by its own rule.
`origin/m0` was already an ancestor of `m42` at the claim, so nothing was merged.
The stand after batch 60 put **Africa at 176 against Asia's 178**, so this batch
is five **Africa** rows and no other lane.*

### The vein chosen, and the table that chose it in one request

Deviation 1450's ranking, re-taken on this fire's head and then asked of Africa
alone:

| the Africa vein | held records inside the 682 | free rows left | of those, dated |
| --- | --- | --- | --- |
| `Q2946372` Mali War | 26 of 31 | 52 | 48 |
| `Q81545` Libyan Civil War | **15 of 17** | 16 | **16** |
| `Q33761` Arab Spring | 6 of 6 | 14 | 14 |
| `Q243620` Somali Civil War | 5 of 5 | 24 | 20 |
| `Q1773926` Congo Crisis | 5 of 6 | 6 | 6 |
| `Q179848` Scramble for Africa | 4 of 10 | 16 | 15 |

**The stand after batch 60 asked a fire to check the Libyan vein before assuming
the Mali one again, and the check is why this batch exists.** The Mali vein is
still the larger number, and batches 59 and 54 both found its remaining rows argue
chronology; the Libyan vein has sixteen free rows and **every one of them is
dated**, which the Mali vein's 52 are not. The ranking also moved under the fire:
`Q81545` reads **15 of 17** and not batch 46's 15 of 15, because two more held
records have acquired a `P361` to it on Wikidata since — the number is somebody
else's database and moves without this atlas changing.

Then the second half of deviation 1450, before an article was opened: each
candidate's `[[wikilink]]` targets folded against the `wikipedia.en` title of
every active event, with `<ref>…</ref>` stripped first. **Eight of the Libyan
vein's sixteen name a held record in prose**, and the five with the most are this
batch.

A11 (b)'s partition check was made before the import, against `origin/m42b`'s
`data/events/` and `data/places/` and against its data for the five qids: none of
the five records and neither of the two places is there.

In date order:

| the record | sitelinks | dates | placed at | filed under |
| --- | --- | --- | --- | --- |
| `battle-of-brega-ajdabiya-road` | 4 | 8 Apr – 11 Aug 2011 | `brega` (held) | `libyan-civil-war` |
| `battle-of-the-misrata-frontline` | 2 | 16 May – 23 Aug 2011 | `misrata` (held) | `libyan-civil-war` |
| `zliten-uprising` | 3 | 9–16 Jun 2011 | `zliten` (new) | `libyan-civil-war` |
| `second-gulf-of-sidra-offensive` | 3 | 22 Aug – 20 Oct 2011 | `gulf-of-sidra` (new) | `libyan-civil-war` |
| `tripoli-clashes-mid-october-2011` | 1 | 14 Oct 2011 | `tripoli` (held) | `libyan-civil-war` |

All five pass **depth** (each carrying `P361` `Q81545`), **span** (each inside the
war's 15 February to 23 October 2011) and **lane** (each deriving `africa`). All
five filed, so **main stays at 239**, unmoved now through thirty-two batches, four
curation fires and A14.

**No class was added.** `Q178561` battle covers three of the five, `Q124734`
rebellion the Zliten uprising and `Q350604` civil war / rebellion the October
clashes; all three have been in the table since M40a or earlier. `Q124734` carries
no category, so **`zliten-uprising` is written with no `category` key at all** —
an import that has none says nothing.

**Two titles are the article's and not the item's** (A12 (5)): `Q7843682`'s label
is the bare "Tripoli clashes" and its article carries the disambiguator that tells
it from the February ones; `Q11681484` capitalises "Uprising" and its article does
not. Both carry `title-from-article`.

**Two intervals were widened under A7 and three were not touched.** `Q2408799`,
`Q11681484` and `Q7843682` state exactly the days their articles state.
`Q3477174`'s only date is a `P585` of 20 October while its lead says the operation
ran "in August and September 2011" and its own account opens on "22 August", so
the start is widened back and the lead's own "ended on 20 October" is the end.
`Q4870546` is **the harder one and batch 60's Motien Pass case again**: its only
date is a `P585` of 18 April, its own description says "fought 8 April to 14 July
2011", and the article's Battle section opens "By 8 April" and runs on to 11
August, when Brega "had fallen under opposition control". A7 widens and never
narrows, so the interval covers both readings — 8 April to 11 August — with the
`date` flag and a note saying which end came from where. A reviewer decides.

### The seven edges, and the two that did not reach the component

Seven edges, every one quoted from an article at the revision in its own locator;
the full table with the sentences is in `docs/m42-connections.md` under "Batch 61".
...
**The largest connected component moves, 682 to 685, and the honest reading is
three of five and not five.** `battle-of-brega-ajdabiya-road`,
`second-gulf-of-sidra-offensive` and `tripoli-clashes-mid-october-2011` joined the
682 through held records that were already inside it. The other two did not,
because **`battle-of-misrata-2011` had no edge at all** — a held battle of 1,500
dead with a degree of zero, which the vein's prose gave away and the ranking table
could not, since deviation 1449's number counts a vein's held records inside the
component and this one was outside it. The frontline battle and the Zliten uprising
attached to it instead, and the three of them are now a component of their own.

**The component count stays at 308 and the edgeless count falls, 232 to 231** —
the first fall in three batches. Nothing was stranded: every arrival had an edge
before it was committed, and one record that had none has two. **The validator's
warning count falls with it, 586 to 585**, which is that same `degree-zero` going
away.

### A9's places, and the point that is at sea

**Two place records are new, three held places were reused, and not one was
hand-corrected.** Both new ones carry `summary: null`, which is what A14 (6) left
every imported place at, and the `a9-place` flag. Neither carries a `wikidata`,
which is how batch 60 wrote `korsakov`, `hsimucheng` and `tsushima-strait`: A9's
steps hand back a point, and the item they read it from is named in the note.

- **`brega` was reused** and it is the cleanest case A9 has had: `Q4870546` has no
  `P625`, its `P276` is `Q36219`, and `Q36219` is the item this atlas has held as
  `brega` since batch 22, with four battles standing on it.
- **`misrata` was reused** and it is **batch 60's `liaodong-peninsula` case read
  the other way round.** `Q2408799`'s `P276` is `Q32845`, the Misrata *District*,
  whose own `P625` is 32.3667, 15.0833 — the city's point to two decimals, 1.5 km
  from the `Q131323` this atlas holds as `misrata`. A9 matches on the item and
  these are two items, so the letter of it would write a second Misrata 1.5 km from
  the first. It was reused instead: **a district whose point is its own
  settlement's is not a neighbouring place**, and the note says so.
- **`tripoli` was reused**, A9's second step from `Q7843682`'s `P276` `Q3579`.
- **`zliten`** is A9's second step from `Q11681484`'s `P276` `Q207259`, a city, so
  `precision: city` off its class `Q515`.
- **`gulf-of-sidra`** is A9's **first** step and the first place this run has
  written at sea. `Q3477174`'s own `P625` is **31.5 N, 18 E**, which falls in the
  middle of the gulf the offensive is named after, 60 km off the coast it ran
  along. The item's `P276` is `Q1376543`, the Via Balbia — the coastal road itself,
  which carries no point at all — so there is nothing to fall back to and nothing
  to reuse. `precision: point`, which is the fallback where no place item supplies
  a class, with the note saying the point stands for the coast from Brega west to
  Sirte and not for a spot on the ground. Its lane derives `africa` by
  **`nearest`** at 0.69°, which is what a point at sea does.

### The actors

**Not one of the five items carries a `P710` at all**, so all five arrived with
`actors: []` and there was nothing for the pass to map, as in batches 50, 52, 56,
58 and 59. `docs/m53-polities.md` §4.1 was re-taken with the batch: **403 of
1,135** by the start rule, the denominator moving and the numerator not.

### What was refused

- **Two records**, and the first is the row that hurts. **`Q1042279`, the Tripoli
  protests and clashes of February 2011, has eight sitelinks and five prose hits
  and no writable edge.** Its background reads *"As the Arab Spring caused
  revolutions in Tunisia and Egypt, which both border Libya, Gaddafi was reportedly
  still in full control"* and then compares the three countries' corruption
  indices: the causal verb in that sentence belongs to the Arab Spring and the two
  revolutions, not to the two revolutions and Tripoli. Everything else it names is
  its own parent, which C8 bars, or enumeration. A record that would arrive
  edgeless was refused rather than written, which is what batches 52 and 59 did.
  **`Q277176`, the Nafusa Mountains campaign, was refused for the arrow of time**:
  its one prose hit is `2011-military-intervention-in-libya` and its one claim
  about it is a recapture *"supported by NATO airstrikes"* — but the article's own
  Campaign section opens on **1 March 2011** and the intervention is 19 March, so
  the edge would run backwards and rule 4 would refuse it. A fire that widens the
  campaign is not what A7 allows, which widens from the article and this article
  gives 1 March.
- **`Q4621705`, the 2011 Libyan rebel coastal offensive, was refused under A9**:
  it carries no `P625`, no `P276`, no `P131` and no `P17`, so **all four of A9's
  steps return nothing** and there is no place and no lane to be had. It is the
  first row this run has refused on the place alone.
- **Four edges**, listed in `docs/m42-connections.md`. The one worth naming here is
  `battle-of-bin-jawad` → `second-gulf-of-sidra-offensive`: the offensive's account
  of 24 August has loyalists ambushing the rebels at Bin Jawad *"in an echo of an
  earlier defeat in March"*. **That is batch 60's Motien Pass refusal in another
  war** — an earlier engagement this one was compared to is not a condition of it —
  and it is now the second time the class has been needed in two batches.

## Batch 62 — the Arab–Israeli conflict's Lebanese fronts, and the six singletons that argue only with their own parent

*25 September, the same fire, continuing under STEP 3's "each imported,
connected, filed, placed, validated, rebuilt and pushed before the next". Batch 61
put Africa at 181 against Asia's 178, so A10's order of need turned over and this
batch is two **Asia** rows and no other lane.*

### The six held Vietnam records were read first, and they gave nothing

The stand after batch 60 said **six held Vietnam records are singletons whose
articles argue edges nobody has written** and that a fire could write them without
importing anything. **This fire looked, and they do not.** Deviation 1450's fold,
turned on records already here rather than on candidates: every one of
`battle-of-hue`, `battle-of-long-tan`, `battle-of-ap-bac`,
`1963-south-vietnamese-coup`, `battle-of-hamburger-hill` and `battle-of-ia-drang`
links in prose to **`vietnam-war` and to almost nothing else**, and `vietnam-war`
is the parent of five of the six. **C8 bars every one of those edges.** It is the
Korean War's finding — 0 of 21 inside the component, every edge the vein offers
running from the war to its own child — in a second vein, and it is now measured
rather than assumed. What else the six link to is comparison, and
`docs/m42-connections.md` lists each one.

**Two of the links that chose those six were not in the prose at all.**
`battle-of-long-tan` → `tet-offensive` and `1963-south-vietnamese-coup` →
`geneva-conference` are **template and navbox links**, which the plain-text extract
does not carry and the wikitext fold counted anyway. **Deviation 1453**: the fold
strips `<ref>…</ref>` and nothing else, so an article's navbox reads as an
argument. A fire ranking candidates should expect a false positive of that shape,
and the cheap check is whether the link survives in the article's plain text.

### The vein, and the number that chose it

The ranking asked of Asia on this fire's head:
...
twenty-nine free rows against the Russo-Japanese War's five. Fourteen of them name
a held record in prose.

A11 (b)'s partition check was made before the import: neither record and neither
qid is on `origin/m42b`.

| the record | sitelinks | dates | placed at | filed under |
| --- | --- | --- | --- | --- |
| `operation-accountability` | 14 | 25–31 Jul 1993 | `lebanon-q822` (held) | `arab-israeli-conflict` |
| `2024-lebanon-electronic-device-attacks` | 46 | 17–19 Sep 2024 | `lebanon-q822` (held) | `arab-israeli-conflict` |

Both pass depth, span and lane; both filed, so **main stays at 239**. **No place
was written**: both stand on the `lebanon-q822` this atlas has held since M42
batch 14, reached by A9's second step from `P276` `Q822`, neither item carrying a
`P625`. **No class was added.** Neither interval was widened: `Q2026496`'s
`P580`/`P582` are the days the article gives, and `Q130314422`'s are 17 and 19
September against the article's 17 and 18, so **the wider pair is kept because A7
never narrows.**

**One record is written with no `category` and it is a first for this run.**
`Q130314422` carries **seven** `P31`, and three of them are in the class table and
disagree: `Q2001676` offensive says `war`, `Q2223653` terrorist attack says
nothing, `Q891854` bomb attack says nothing. All three agree the kind is an event,
so the record is not refused; **choosing between "war" and "terrorist attack" for
the pager explosions is not an import's to make**, and the note says so. The other
four classes are not in the table at all, which changes nothing, because a class
that decides nothing is what the three deciding ones already are on the category.

### The three edges

All three `precondition-of`; the sentences are in `docs/m42-connections.md`.
...
**The largest connected component moves, 685 to 687, and this time it is two of
two.** The middle edge is the one worth having: `taif-agreement` →
`operation-accountability` → `operation-grapes-of-wrath` is **a chain of three
across seven years**, an accord that disarmed every Lebanese militia but one, the
operation launched against that one, and the operation launched when the first
one's ceasefire failed — and each of the three held records had a degree of one
before this batch. The component count stays at 308, the edgeless count stays at
231, the warning count stays at 585.

**Neither item carries a `P710`**, so both arrived with `actors: []`;
`docs/m53-polities.md` §4.1 reads **403 of 1,137**.

### What was refused, and the one that is worth a fire's while

- **`Q725043`, Operation Opera, 39 sitelinks — the most valuable row this fire
  refused.** Outside its own parent its prose names `iran-iraq-war` and `gulf-war`
  and both are refusals: the Iranian strike on the same reactor came *"shortly
  after the outbreak of the Iran–Iraq War"*, which dates that strike and not this
  one, and the Gulf War appears as what later destroyed the ruins and in two
  attributed opinions, Begin's that the war *"vindicated his decision"* and
  Cheney's that the bombing *"made Desert Storm easier"*. **An attributed opinion
  that an earlier act was vindicated is not the article stating an edge**, and it
  is a refusal class a fire should expect on any strike whose worth is argued over.
- **`Q59417678`, Operation Northern Shield, and the edge the atlas cannot take
  yet.** Its background states one plainly: *"After the 2014 Gaza War, which saw
  numerous tunnels dug by Hamas from the Gaza Strip into Israel being uncovered
  and being utilized in several attacks, the IDF renewed its search for Hezbollah
  ...
  `gaza-war-2008-2009` and `gaza-war` and nothing between them — so the edge has no
  `from`. **A fire that imports `Q17061507` gets this edge with it**, and that is
  the most concrete thing this fire found for the next one.
- **`Q97860712`, the 1991 Iraqi missile attacks against Israel, refused on the
  class and not on the argument.** Its two `P361` are `Q37643` the Gulf War and
  `Q8669`, **both held**, which would have been this run's first two-parent filing
  under A8. Its `P31` is `Q111034471`, missile strike, **which is not in
  `data/imports/wikidata-seeds.json`**, and an item of a class nobody has decided
  about is refused and listed rather than guessed at. **Adding that class is an
  edit somebody should argue with and not a fire's**, so it is named here for them,
  with the reading it would need: a missile strike is organised armed force between
  polities, which is what `data/categories.json` describes as `war`.
- **`Q28980421`, the Israeli annexation of East Jerusalem, refused on the date.**
  Its `P585` carries **three values that disagree** — June 1967 at month
  precision, 1967 at year precision and 30 July 1980 at day precision — which is
  not a span recorded by one of its ends but an item saying three things. A7 widens
  from the article and never picks between an item's own readings.
- **`Q130314422`'s umbrella siblings were left alone.** `Q151622` the
  Israeli–Palestinian conflict (88 sitelinks) and `Q3241199` the Israeli–Lebanese
  conflict (28) are the two highest rows in the vein and both are **umbrellas over
  records this atlas already holds**. Importing one would put an umbrella above a
  dozen held records without re-filing any of them, and **re-filing the corpus is
  not a batch's to do**: A6 and A8 say a filing writes every umbrella that fits,
  which is a statement about the record being written and not a licence to move
  held ones. They are named here as the two rows a fire should not take casually.
- **Four more candidates** and their reasons are in `docs/m42-connections.md`.

## Where the run stands after batches 61 and 62, for the fire that picks it up

*25 September, the fire that claimed at 14:51Z. **Two import batches in one
fire**: batch 61 (Africa, five records) and batch 62 (Asia, two). Seven events,
ten edges, two places, no class added, and seven leads cached under
`tools/import/cache/wikipedia/` at the revisions the records cite, so A14 (3)'s
invariant holds over everything this fire wrote. No code changed and no data
vocabulary either.*
...
| | |
| --- | --- |
| corpus | **1,137 active** (+7 over the fire, +2 on this batch) |
| **main** | **239**, unmoved through thirty-three batches and four curation fires |
| filed | 898 (+7) |
| active edges | **1,010** (+10 over the fire, +3 on this batch) |
| largest connected component | **687** (+5 over the fire, +2 on this batch) |
| second component | **13**, the Korean War's earlier battles, unmoved |
| components | **308** (unchanged over the fire) |
| events with no edge at all | **231** (−1 over the fire) |
| validator | **0 errors, 585 warnings** (−1 over the fire) |
| per lane, active | **Europe 466**, Africa 181, Asia 180, Americas 310 |
| per lane, main | Europe 88, Asia 64, Americas 56, Africa 31 |

**A10's order of need: Asia trails at 180** against Africa's 181, so the next
import batch is Asia's unless a chain crosses out of it — and batch 62 named the
Asia row to take.

**What is open, in the order a fire should weigh it:**

- **Import `Q17061507`, the 2014 Gaza War, and batch 62's refused edge arrives
  with it.** The Northern Shield article states *"After the 2014 Gaza War, which
  saw numerous tunnels dug by Hamas from the Gaza Strip into Israel being uncovered
  and being utilized in several attacks, the IDF renewed its search for Hezbollah
  tunnels in northern Israel"* — a clean `precondition-of` with no `from` here, in
  an atlas that holds `gaza-war-2008-2009` and `gaza-war` and nothing between them.
  It is Asia, it is the trailing lane, and the vein is `Q8669`, whose held records
  are 12 of 12 inside the component after this fire. **This is the most concrete
  thing two batches of looking produced for the next fire.**
- **Rank the veins before reading anything** (deviation 1450), and re-take the
  table each fire because it moves with every batch *and with Wikidata*: `Q81545`
  read 15 of 15 in batch 46 and 15 of 17 here, because two held records have
  acquired a `P361` since. One POST to the query service, the qids of every active
  event inside the largest component in a `VALUES` block, grouped by `P361`. **Then
  fold the candidates' prose links before opening an article** — but **strip more
  than `<ref>`** (deviation 1453): Long Tan's link to `tet-offensive` and the 1963
  coup's to `geneva-conference` are navbox links and neither is in the prose. The
  cheap check is whether the link survives in the plain-text extract.
- **After this fire the Asia ranking reads: Cold War 21 of 23, Arab–Israeli
  conflict 12 of 12, Vietnam War 10 of 19, Afghan Conflict 7 of 8, War on Terrorism
  7 of 8, Indo-Pakistani wars 5 of 5, Korean War 0 of 21.** The Africa ranking
  reads Mali War 26 of 31, Libyan Civil War 17 of 19, Arab Spring 6 of 6, Somali
  Civil War 5 of 5, Congo Crisis 5 of 6, Scramble for Africa 4 of 10. **The Somali
  Civil War is the untouched Africa vein with the best shape**: five of five inside
  the component, twenty-four free rows and twenty dated, and `Operation Gothic
  Serpent` at its head with **six prose hits including `battle-of-mogadishu-1993`,
  `unified-task-force` and `united-nations-operation-in-somalia-ii`** — the richest
  single candidate either ranking turned up this fire and untouched by any batch.
  The **Congo Crisis** has `Operation Grandslam` (four hits) and `Niemba ambush`
  (two) beside it.
- **The six Vietnam singletons are settled and a fire should stop returning to
  them.** They argue only with `vietnam-war`, which is their parent; C8 bars it.
  What frees them is C8 and not a fire's reading.
- **`battle-of-misrata-2011` had a degree of zero and the ranking table could not
  see it**, because deviation 1449's number counts a vein's held records *inside*
  the component. Batch 61 found it in the vein's prose and gave it two edges, and
  the three of them — it, `battle-of-the-misrata-frontline` and `zliten-uprising` —
  **are now a component of their own, outside the 687.** What would join them is an
  edge from the Misrata front to the Battle of Tripoli or to Bani Walid, and batch
  61 refused the one the article offered ("intended to capture the city of Bani
  Walid within days" is an intention, three weeks early). **A fire looking for a
  cheap gain should read `battle-of-tripoli-2011`'s own article**, which the
  February Tripoli article says the Nafusa and Zawiya advances preceded.
- **The two Arab–Israeli umbrellas are a trap and not an opportunity.**
  `Q151622` (88 sitelinks) and `Q3241199` (28) sit above a dozen records this atlas
  already holds. Importing either adds an umbrella and re-files nothing, and
  **re-filing held records is not a batch's to do**.
- **`Q111034471`, missile strike, is a class the table has not decided**, and it
  cost this fire `Q97860712`, whose two `P361` are both held and would have been
  the run's first two-parent filing under A8. **Adding it is an edit somebody
  should argue with.**
- **C8 is still the largest thing standing between this atlas and the chains the
  owner asked for**, and two veins now measure its cost exactly: the Korean War
  reads **0 of 21 inside the component** and the six Vietnam singletons argue
  nothing else at all. One edge `korean-war` → `first-battle-of-seoul` would join
  eighteen.
- **`reacted-to` reads backwards on the card, on all 65 of them** (deviation 1443,
  and batch 61 wrote two more). One word, a display change, and every reader of one
  of those cards is told the reverse of what the record says.
- **`ilinden-preobrazhenie-uprising`'s own lane is simply wrong** and a fire could
  fix it by clearing the record's `region`; `execution-of-the-romanov-family`,
  `battle-of-korsakov` and `japanese-invasion-of-sakhalin` are deviation 1423's
  question and are not a fire's. **`declaration-by-united-nations`'s cache has
  drifted from its citation** and wants a re-cache under A14 (3)'s pass. **Two of
  the fifteen tombstones still want an A6 umbrella and not an edge**:
  `1948-palestine-war` and `balkan-wars`. **The fourteen placeholder summaries are
  not a fire's to fix.**
- **The next curation fire is the first fire after 02:00Z on 26 September**, and it
  owns A13's relations pass over every active event.
- **Deviation numbers: take the next above 1453.** This fire wrote **1453** and
  nothing else: batch 61 met no new class of problem, and batch 62's finding is the
  one number.

**2,114 tests pass locally, 1,819 pure and 295 browser, with nothing failed and
nothing skipped**, run the way the check runs them since M63 on the final tree.
One suite failed on a first local run in each batch and both times it was
`tests/m53.test.mjs` §4.1's denominator, which moves with the corpus and was
re-taken with each batch — 1,130 to 1,135 to 1,137, the numerator staying at 403
