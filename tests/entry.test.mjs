...
import { atlasOf, FIXTURE_DATA, fixtures as fixtureRecords } from './helpers.mjs';

// The page is handed an atlas and does not know what it was read out of. Up
// to H3c that was worth running every test twice for, once over each of the
// two graph files (A12); there is one of them now. The two tests that build
// no atlas at all are above.
...
test('a body renders with a table of contents, record links and numbered marks', async () => {
  const html = await page('fixture-event-a', 'event');
...
test('a record with no entry says so and invites one', async () => {
...
test('an actor entry lists relations and appearances; a place entry lists what happened there', async () => {
...
test('nothing from a record reaches the page unescaped', async () => {
...
test('a tombstone entry still resolves and says what it is', async () => {
  const { byId } = await fixtureRecords();
