# Status, as it stood on 5–6 September 2026

Everything `STATUS.md` carried before H8 cut it down, verbatim and in its
original order: the run-by-run position from M0 to H7, the phase, the
decisions taken, the full "Next" and "Open questions" lists, deviations 1 to
297, the dates to verify, and where things live. Nothing here has been
edited — a deviation numbered here is the same deviation the run wrote.

`STATUS.md` is the position and this is the record behind it (health review
B, finding 35). New deviations are numbered on from 297 there, not here.
...
A, finding 8). [H9, 6 September: that last clause was not true when it was
written. `isDraft` reads `review.status`, and neither import wrote one — the
Wikidata import wrote a flag and no status, the CShapes import no `review` at
all — so every record they created stayed out of the queue. The closing
review found it (R10) and H9 made the sentence true; it is left standing with
this note rather than rewritten, because what the run believed is part of the
account.]
