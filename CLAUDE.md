...
data/index/manifest.json   GENERATED, never cached: counts, the hashed names below, roles, land epochs, presence shards
...
Eight record kinds: `event`, `edge`, `source`, `actor`, `place`, `relation`,
`presence`, `narrative`. An
actor is a person, polity, institution or people; events name the actors *of*
the event with the role each played (`actors: [{ actor, role }]`). Roles are
free text for now and `build-index.mjs` reports the set in use; an actor is
never on the timeline alone, so it has no lane.
