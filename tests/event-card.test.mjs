...
// it was built from. Both datasets on both paths — only the fixtures hold a
// retracted edge, only the repository a merged event (A12).
for (const [label, buildAtlas] of ATLAS_BUILDS) {
  const atlas = await buildAtlas(path.join(ROOT, 'data'));
  const fixtures = await buildAtlas(FIXTURE_DATA);
  const carnation = atlas.events.get('carnation-revolution-1974');

  test(`the head carries the actors as chips, with the role in the title attribute, over ${label}`, () => {
  ...
  // nothing fetched.
  test(`the sections count what the topology and the sources index actually hold, over ${label}`, () => {
    const html = eventCardHtml(context(atlas), { event: carnation, found, state: state() });
  ...
  test(`other branches is there only while a path is being walked, over ${label}`, () => {
  ...
  test(`a disputed link is counted in the header of the closed section it is in, over ${label}`, () => {
  ...
  test(`walking a chain opens Consequences and draws the path as a breadcrumb, over ${label}`, () => {
    const alvor = atlas.events.get('alvor-agreement-1975');
    const chain = ['carnation-revolution-1974--alvor-agreement-1975--caused'];
  ...
  test(`arriving from a source card opens Sources; otherwise the remembered choice stands, over ${label}`, () => {
  ...
  test(`the disputed notice opens the section that holds the argument, over ${label}`, () => {
  ...
  test(`a walk cut short by a retracted step says so on the card, over ${label}`, () => {
    const carnation = atlas.events.get('carnation-revolution-1974');
  ...
  test(`nothing a record carries reaches the card unescaped, over ${label}`, () => {
    const nasty = {
