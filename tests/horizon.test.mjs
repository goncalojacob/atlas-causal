...
// Every test twice: the horizon is a graph query and a panel over it, and
// neither can tell whether the atlas was built from the topology or from the
// spine — which is the whole of what H3b relies on (A12).
for (const [label, buildAtlas] of ATLAS_BUILDS) {
  const atlas = await buildAtlas(dataDir);
  const ctx = context(atlas);

  test(`the horizon year is the reader's own, or the window's far end, over ${label}`, async () => {
  ...
  test(`the reachable set is lit only when a year and an event were chosen, over ${label}`, async () => {
  ...
  test(`the panel lists what graph.js found, and each line walks to it, over ${label}`, async () => {
  ...
  test(`the default horizon is drawn closed and offers no way back, over ${label}`, async () => {
  ...
  test(`an event that led to nothing by the year says so, over ${label}`, async () => {
    const event = atlas.events.get(REVOLUTION);
