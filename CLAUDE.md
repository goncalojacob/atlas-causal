...
src/layer-control.js       the map's whole legend and there is no other: `territories` and `events` as rows, then a collapsed `<details>` per group — "base map", one toggle and one swatch per switchable layer of `manifest.base.layers`, and "events by category", one toggle and one glyph per category in use. Four targets in the phone drawer and not nineteen. Built rather than written into index.html because every label and id comes from `data/`; writes the whole `LAYERS` order so the default writes no link, with M30b A11's token set in the place of `events` — the bare `events` while every category is on and one `events:<id>` per category still on when one is not
src/categories.js          pure: what a `?layers=` list says about categories — whether the events layer is on at all, which categories are still on, and the tokens a toggle writes — and the labels and counts the manifest carries. The removal itself is one line in emphasis.js, where the lens is
