...
*The base map is six layers at two levels* ●. `data/geo/base/`, built by
...
**Six layers**: `coast`, `rivers`, `lakes`, `physical` (deserts, plateaux,
ranges and the other landforms, by a frozen allow-list of `FEATURECLA`),
`mountains` (the elevation points, each with the height the file gives it)
and `cities`. The first four are geometry; the last two are **arrays of small
objects and not GeoJSON**, because a `Feature` around a point is about a
third scaffolding and neither is ever drawn as a shape.
