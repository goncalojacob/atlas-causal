...
test('manifest names the hashed files, counts, lanes and land', async () => {
  ...
  //
  // Counted off the records rather than written out: every one of these moved
  // when M43b stretched the fixtures to 2025, and what the manifest has to get
  // right is the corpus it was built from, not a set of numbers from the day
  // the test was written.
  const corpus = await fixtures();
  const kinds = ['event', 'edge', 'source', 'actor', 'presence', 'place', 'relation', 'office', 'tenure', 'narrative'];
  assert.deepEqual(manifest.counts, {
    ...Object.fromEntries(kinds.map((kind) => [
      `${kind}s`, corpus.records.filter((r) => r.kind === kind).length,
    ])),
    regions: corpus.regions.length,
  });
...
test('the manifest carries the base map that is on disk, cell by cell', async () => {
  ...
  for (const id of ['rivers', 'lakes', 'physical', 'mountains', 'cities']) {
    ...
    assert.deepEqual(entry.cells.map((c) => c.key), ['x2y2'], `${id}: the fixture places' box`);
    for (const cell of entry.cells) {
      const text = await readFile(path.join(FIXTURE_DATA, ...cell.file.split('/')), 'utf8');
      assert.equal(cell.bytes, Buffer.byteLength(text, 'utf8'), `${cell.key} is the bytes on disk`);
    }
    // Never a file that is not there: a manifest entry M37 cannot fetch is a
