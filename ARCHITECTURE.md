...
Revision 9, 3 September 2026. Sections marked ● exist in v1; things marked ○
...
│   ├── state.js                  ● { from, to, view, selected, place, actor, chain, layers } ⇄ URL; data-free
...
│   ├── panel/event.js  place.js  actor.js  cluster.js   ● one card each
│   ├── contribute/
...
| `panel/` | The shell plus one file per card. Detail, consequences, convergence, supporting and dissenting citations shown apart, confidence and status shown as such; an event's actors with their roles, an actor's card, a place's card, and the members of a cluster. | Traversal logic. |
