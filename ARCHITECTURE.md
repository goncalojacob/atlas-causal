...
│       ├── topology-<hash>.json  ● every event {id,title,when,where,region,status,actors}, every edge {id,from,to,type,confidence,status}, every actor {id,actorType,name,names,when,status}
