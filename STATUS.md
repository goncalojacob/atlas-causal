...
H3b. Code only; nothing under `data/` moved and `data/index/` was not
rebuilt. 772 tests.

**The round trip is not a list of fields, it is the suites.** A hand-written
list of what the spine must carry drifts from what the cards actually read.
So the seven suites that build a real atlas — the event, actor, place and
source cards, the entry page, the horizon and the graph queries — now run
twice, once over `createAtlas` and once over `createAtlasFromSpine`, over
both the fixtures and the repository's own data: 45 tests where there were
22, and every assertion they already made is now an assertion about the
spine (A12). Beside them, what those suites cannot see: the A11 surface, the
edge tuple expanded back with its id and its status, `resolve` over every id
and alias including the merge hops, the two citation counts agreeing on
every event, actor and place, the search box answering the same queries in
the same order off either atlas (A9), and `loadSpine`'s fetching discipline
— the manifest `no-store` every time, the hashed spine once, a rejection
dropped rather than kept as the answer.

**The three differences, and where each is handled.** An edge is five slots
in the spine and an object in the atlas, so the loader synthesises
`from--to--type`, which is why `edgeId` moved to the leaf `vocab.js` beside
the pattern it inverts (deviation 219). `regionMethod` and `presenceType`
are gone, and nothing asked for them. And a record says on itself how many
citations it makes, so `citationCount` is read rather than counted on the
spine path (deviation 220) — which is what lets the citer rows leave the
sources index in H3b. The one thing the spine cannot answer at all is the
other direction, which records cite a source: `retractionPlan` now takes
those rows pre-fetched, as `topology.citers` (deviation 221).

Before that, H3a-1 (`docs/health/h3a-brief.md`, amendments A0–A14), the
health cycle's fifth run: **the index emits the spine, the search shard and
the citer directory beside the topology, which is unchanged.** Code only;
nothing under `data/` moved, `data/index/` was regenerated by that run, which
owns it. 706 tests.

**What is in `data/index/` now**, measured on this dataset — 329 events of
which 137 active, 161 edges, 412 actors, 710 presences, 34 sources — raw and
gzipped, which is what Pages serves:

| File | Raw | Gzipped |
|---|---|---|
| `spine-075c2ac9f377.json` | 791.4 KB | 61.0 KB |
| `search-d9d315255367.json` | 217.7 KB | 22.4 KB |
| `citers-53fa8cafa5d4/`, 33 files | 255.7 KB | 18.4 KB |
| `topology-a3768c04ac37.json`, unchanged | 905.5 KB | 65.4 KB |
| `sources-aa684b3c2ad4.json`, unchanged | 328.8 KB | 22.7 KB |
| `review-326d91357c8f.json`, unchanged | 258.9 KB | 19.7 KB |
| `manifest.json` | 6.1 KB | 1.9 KB |

**The numbers are smaller than the plan projected, and that is worth saying
plainly.** A13 projected a 522 KB spine and a 2.5× improvement; the measured
spine is 791.4 KB. Once H3b moves the pages over, first paint goes from
1,240 KB to 827 KB raw and from 88.1 KB to 64.7 KB gzipped — about 23 KB
over the wire. Nearly all of that is the citer rows leaving the sources
index (328.8 KB → 29.4 KB, measured; deviation 216 says why the strip waits
for H3b). The file every page loads *whole* goes 905.5 → 791.4 KB, 1.14×, so
the wall this project actually runs into — the whole graph in memory,
because convergence cannot be answered from a window — barely moves. The
next lever is the presences: 373.8 KB, 47 % of the spine, needed by nothing
until the territory layer draws. That is H4a's question.

**What the spine is.** A projection of the topology, not a second reading of
the records: `ARCHITECTURE.md` carries the field table. `when` is carried
verbatim rather than reduced to a pair of astronomical years, because
fourteen readers want the object and `formatYear` would print 1 BCE as "0".
An edge is `[from, to, type, confidence, status]` — five elements, the fifth
being what keeps a retracted argument out of consequences and convergence —
with the id synthesised as `from--to--type`, which every one of the 161 edge
ids equals exactly; an edge carrying an alias or a merge hop is written whole
instead and the loader will take either. `regionMethod` and `presenceType`
are dropped because nothing draws them, no prose is written into any index
file, and **there are no period shards** (A4): the whole shard payload
measured 288.6 KB, less than one shard's own budget, against four cards that
are not windowed at all. `citesCount` — how many citations a record makes —
now has a name of its own, distinct from a source's `citationCount`.

**The file machinery grew a directory.** `readIndex` walks `data/index/` one
level deep and keys by the relative path; `writeIndex` creates a directory it
needs, prunes inside the one the build names and takes away any other;
`HASHED` knows the spine and the search shard. Without that, `compareIndex`
would have reported `missing spine-<hash>.json` on every run — rule 16, exit
1, and with it the deploy's assert step and every batch of the Wikidata
import. The citers are one hashed directory rather than 34 hashed files
because `manifest.json` is fetched `no-store` on every page load.

Nothing read any of it at the end of that run; H3a-2 above is the loader,
and H3b moves the pages over one commit each.

Before that, H2 (`docs/health/h2-brief.md`), the health cycle's fourth
...
18. **Next build: H3b** (`docs/health/h3a-brief.md`, amendment A0). H3a is
   done, both halves: H3a-1 emitted the spine, the search shard and the
   citer directory, and H3a-2 taught the loader to read them. What is left
   is the switch — the pages move to `loadSpine()` and
   `createAtlasFromSpine()` one commit each, `loadNarratives` stops reading
   the whole topology, the source card fetches its own citer file, and the
   citer rows finally leave the sources index (deviation 216), which is the
   328.8 → 29.4 KB the measurement above is waiting on. Nothing before that
   commit reads the spine in the browser. H5b may run beside it on its own
   branch, without committing `data/index/`.
...
196. **The alias resolution `citationsOf` needed went into `core.js`.** H5a's
...
197. **`lookup` in the rules stops at the alias and does not follow a merge.**
...
198. **The gate's `--index` made the contribution Action rebuild the index.**
...
215. **A tombstone keeps whatever its kind calls its label.** A10 gives the
     tombstone list as `title`, `when`, `place`, `region`, `wikidata`,
     `status`, `supersededBy`, `aliases` — written for events, which are the
     only kind that has a tombstone in either dataset today (175 retracted
     and 17 merged in the repository, one merged in the fixtures). Applied
     literally to an actor or a place, whose label is `names`, it would
     leave a merged record with no name for the panel to say it was merged
     *from*. So the list also carries `name` and `names`. It costs nothing
     today and it is one line to take back out.
216. **The citer rows do not leave the sources index in this run.** A3 has
     `sources-<hash>.json` losing `citations` to the citer directory, and
     A14 asks for `node --test` green with no test edited to pass. Both
     cannot hold: the strip was tried and fails nine tests in three cards —
     the source card's row count, the event card's section counts, the
     sections' own count, the citations review, and the two that check
     `citesCount` against the atlas — because `createAtlas` builds
     `citationCount` from `source.citations` and this run does not touch
     `src/data.js`. The directory is emitted beside the rows; the strip
     belongs to the commit that teaches the card to fetch it, which is
     H3b's. Measured, so the number is not lost: the sources index without
     its citer rows is **29.4 KB** against 328.8 KB with them.
217. **A source nothing cites gets no citer file.** A7 says one file per
     source; an empty one would be a request that answers nothing, and the
     `citationCount` in the sources index already tells a card there is
     nothing to fetch. 33 of the 34 sources have a file.
218. **`writeIndex` prunes what it can name, not everything it finds.** A6
     says it deletes every file and directory a fresh build does not name.
     Taken literally that would also delete anything a person or another
     tool had put in `data/index/`, silently, since `readIndex` never
     reports those as stale. It removes hashed files and `citers-<hash>`
     directories the build does not name — which is every case A6 and A7
     describe — and leaves anything else alone.
219. **`edgeId` moved from `validate/core.js` to `vocab.js`.** The loader
     has to synthesise `from--to--type` to expand an edge tuple, and
     `core.js` is the validator: importing it from `data.js` would pull the
     whole of it — the schema compiler, the rules — into every page, on a
     milestone whose subject is what a page downloads. `vocab.js` is a leaf
     that imports nothing and already holds `EDGE_ID`, the pattern this
     function is the inverse of, so the two now sit together and cannot
     come to disagree. `core.js` re-exports it; nothing else changed.
220. **`createAtlas` gained a flag rather than a twin.** The one thing the
     two atlases genuinely differ about is where "how many citations does
     this record make" comes from: counted out of the sources index on the
     topology path, read off `citesCount` on the spine path (A8). A second
     assembly function would have been four hundred lines duplicated for
     that; `createAtlasFromSpine` expands the spine into the shape
     `createAtlas` already reads and passes `citesCount: true`. The two
     answers are asserted equal for every event, actor and place in both
     datasets.
221. **`retractionPlan` takes the pre-fetched citers map in this run.**
     Item 18 above, written by H3a-1, left it to H3b with the strip. It is
     three lines and it is the loader's half of A7, so it went here with
     the rest of the loader; the strip itself is still H3b's, and the
     sources index is untouched. `topology.citers` is read only when a
     caller passes it, so `review/main.js` behaves exactly as before.
222. **The four members A11 excuses are reproduced anyway.** A11 says
     `manifest`, `presenceShards`, `citationsOf` and `aliases` are used
     nowhere outside `data.js` and need not be reproduced. Building on
     `createAtlas` means they come for free, and dropping them would have
     cost a deletion pass and made the two atlases differ in a way no
...
248. **Rule 5's DAG check is still one pass over every active edge, per
...
249. **`checkRules` costs more per call than it did when nothing is reused,
...
250. **The local server rebuilds the topology after each save rather than
...
251. **The palette is memoised in the process, not against anything on
     ...
     needed.
252. **A record under validation is looked at after the topology's rows, not
...
253. **The store does not notice an edit made to `data/` behind the
...
254. **The save endpoint's `index` field is a state, not a list of files.**
