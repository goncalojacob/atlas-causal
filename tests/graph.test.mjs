...
// Every query twice: the graph is the one thing the whole project is for,
// and its answers must not depend on which file the events and the edges
// were read out of — the edge tuple in the spine carries `status` precisely
// so that a retracted argument stays out of them (A2, A12).
for (const [label, buildAtlas] of ATLAS_BUILDS) {
  const adjacencyIn = async (dataDir) => (await buildAtlas(dataDir)).adjacency;

  test(`adjacency ignores retracted edges and keeps tombstones without edges, over ${label}`, async () => {
  ...
  test(`consequences and antecedents are direct and ordered by type, then confidence, over ${label}`, async () => {
    const adj = await adjacencyIn(FIXTURE_DATA);
  ...
  test(`ancestors and descendants are transitive and exclude the start, over ${label}`, async () => {
    const adj = await adjacencyIn(FIXTURE_DATA);
  ...
  test(`convergence excludes only the walked path and reaches through it, over ${label}`, async () => {
    const adj = await adjacencyIn(FIXTURE_DATA);
  ...
  test(`convergence results are ordered by type, confidence, then depth, over ${label}`, async () => {
    const adj = await adjacencyIn(FIXTURE_DATA);
  ...
  test(`the shortest path outward is by hops, and it is the same path twice, over ${label}`, async () => {
    const adj = await adjacencyIn(FIXTURE_DATA);
  ...
  test(`the horizon cuts what is reported, ordered by path length then year, over ${label}`, async () => {
    const adj = await adjacencyIn(FIXTURE_DATA);
  ...
  test(`each result carries the first step of its path and whether any step is disputed, over ${label}`, async () => {
    const adj = await adjacencyIn(FIXTURE_DATA);
  ...
  test(`25 April, horizon 2011, on the repository dataset, over ${label}`, async () => {
