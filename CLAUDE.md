...
node tools/new-record.mjs event|edge|source|actor|place|relation …   # scaffold a record; the text is yours to write
...
schema/v1/                 event, edge, source, actor, place, relation, presence, region, bundle; import-map is tool-side
...
src/panel/event.js         one card each: event.js, source.js, place.js, actor.js, cluster.js;  horizon.js  the "led to by year X" section; the actor card also lists its relations, both ways round
...
Seven record kinds: `event`, `edge`, `source`, `actor`, `place`, `relation`,
`presence`. An
