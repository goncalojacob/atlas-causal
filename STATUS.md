...
429. **The world candidate list is `docs/wikidata-candidates.md`, not
...
430. **Twenty-nine of the 120 ticked candidates are not imported, because a
...
431. **`data/index/` moves on `world`, because the Action commits it.** The
...
432. **M40b's counts, by decade.** Imported is M40a's 91; every one of them is
...
433. **The three decades that barely wire say something about the import, not
     ...
     name one of the twenty-nine deviation 430 lists.** Importing those
     twenty-nine — which needs the change to `tools/import/wikidata.mjs` that
     deviation 430 leaves to the owner — would turn much of the retraction
     list back into records with edges, and is the single highest-value thing
     that could be done to this dataset next.

434. **`role` is from `docs/roles-mapping.md`'s closed list of 31, and carries
...
435. **A genocide record names the perpetrator and not the victims**, which is
...
436. **The Gaza war and the Gaza genocide are kept as two records with one
...
437. **`region` was corrected on wired records where the import's derivation
...
438. **`node --test` is green in the working tree and red on the branch as
     pushed**, for exactly the reason deviation 431 gives. Two tests —
     ...
     deviation 431 already plans for, is what makes the branch green as
     committed.

439. **M41a stopped unfinished: GitHub Actions stopped starting jobs, and the
...
440. **Why the two finished candidate rounds were not ticked.** The rule M41a
...
441. **What the refusals actually cost, and the fix that is pushed and
...
442. **The import Action lost five and a half hours to a hung test, and the
...
443. **The import was writing two fields the contribute form cannot carry
...
444. **`world` is red on two tests, on purpose, because its index is
...
445. **The second Portuguese round returned institutions, not events.** The
     ...
     companies.
446. **An actor takes no edge, so M41b's rule is not M40b's.** The brief says
...
447. **`data/roles.json`, `data/categories.json` and the `category` field are
...
448. **There is no `retraction: { on, reason }` in this schema.** The brief
...
449. **Two of the forty-two were neither wired nor retracted but merged, and
...
450. **M41b's section is a comment on pull request #1 and not in its body.**
