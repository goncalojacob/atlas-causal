...
Revision 10, 3 September 2026. Sections marked ● exist in v1; things marked ○
...
│       ├── topology-<hash>.json  ● every event {id,title,when,place,region,status,actors,weight}, every place {id,name,names,where,region,status}, every edge {id,from,to,type,confidence,status}, every actor {id,actorType,name,names,when,status}, every presence {id,actor,presenceType,dependencyOf,dependencyKind,when,geometry,capital,confidence,status}
│       └── sources-<hash>.json   ● every source record, with `citations` — who cites it, with locator and dissent — and `citationCount`
│
├── schema/
│   ├── v1/event.json  edge.json  source.json  actor.json  place.json  presence.json  region.json  bundle.json  ●
...
│   ├── state.js                  ● { from, to, view, selected, source, place, actor, chain, horizon, layers } ⇄ URL; data-free
│   ├── data.js                   ● manifest → topology (whole) → record text on demand; lookup tables, adjacency, events by actor and by place, an event's point through its place
...
│   ├── new-record.mjs            ● scaffold a record locally, of any of the five written kinds; --new-place writes the event and its place at once
...
source, place, actor, chain, horizon, layers }`, mirrored to the URL so every
view is a shareable link. `horizon` is the exception that proves the rule: it
...
| `panel/` | The shell plus one file per card. Detail, consequences, the horizon, convergence, supporting and dissenting citations shown apart, confidence and status shown as such; an event's actors with their roles, a source's card with everything that cites it, an actor's card, a place's card, and the members of a cluster. | Traversal logic. |
...
that no event references **and that holds no territory**; a place no active
event references; an event dated
