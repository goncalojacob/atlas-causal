...
| file | what it is |
|---|---|
| `entities.json` | `wbgetentities` for the synthetic items, one of each case the import has to handle — including one dated event that names no place and carries no coordinate, which is the case the lane table exists for |
