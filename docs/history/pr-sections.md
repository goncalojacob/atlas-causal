...
**Checks.** `node tools/validate.mjs --index`: **10,553 records, 0 errors**, 1,210 warnings. `node --test --test-timeout=120000`: **0 failures, 0 skipped**, of which 18 tests are new, all in `tests/m50.test.mjs` and all correspondences between `docs/m50-chains.md` and the records rather than states. Eight existing tests were corrected and each replaced a coincidence with the rule it stood in for — the corpus is five centuries long now, so `opensOn` answers where the whole extent used to, and the tenure strip prints `stripScale` rather than the extent it used to coincide with. No display file changed; no new record type, confidence value, edge type, hex value, token or type size; no runtime dependency and no build step; nothing merged into `main`.

**What to look at first.** The three disputed edges, because they are the test of whether the interface shows a disagreement instead of asserting a line. Then "the arrow that is not here" in `docs/m50-chains.md`, which is the decision most likely to be argued with. Then the four §5 screenshots, in the order graph, focus, window, timeline.
