...
wrote data and no pixel, and what M37 and M38 will draw from is now on disk.

*The base map is seven layers at two levels* ● (six until M45b added the relief bands). `data/geo/base/`, built by
`tools/import/naturalearth.mjs` from Natural Earth 10 m committed under
`vendor/` — see **The base map** below for the grid, the two levels, the
per-feature `z` and the `base` block of the manifest. The short of it: a
**far** level is one file for the whole world and is what a reader sees
before anything else arrives; a **near** level is the same layer again at
finer detail, cut on a fixed twenty-four-cell grid and fetched a cell at a
time as the viewport enters one. The coastline's far level is
`land-present.json`, which keeps its name and its manifest key, and it is
still the only base-map bytes before the first picture.

*Which Natural Earth city a place record is, is data* ●.
`data/imports/naturalearth-places.json`, keyed by Natural Earth's own
`NE_ID`, each entry naming a place record. It decides the second half of the
cities layer's rule — over a hundred thousand, **plus every populated place
this atlas names** — and it is where a match a tool could not prove is
written by a person. `data/imports/` is not in the deploy artifact, so the
import copies `place` onto the city itself and the browser never asks for
the mapping.
...
*A territory's border is drawn inland only* (M39b, the owner's screenshot of 5
September 2026). CShapes draws its own coastline and it is not Natural Earth's,
so a territory stroked all the way round put a second shore a few tenths of a
degree from the one `land.js` draws. Which boundaries are inland is the
topology's own answer and is settled at import: an arc two features share, of
different entities, whose dates overlap, is a border between two territories,
and every other arc is a shore, the edge of the dataset, or a boundary with a
state the source does not carry. Each shard of `data/geo/presences/` therefore
holds `arcs` beside `features` — every such border once, whichever of the two
sides names it — and `map/layers/presences.js` draws a territory as two
elements: the closed outline, filled and clicked, and over it a stroke along
those arcs alone. A border a simplified outline no longer runs along is not
written, because a stroke with no fill under it is a border around nothing.
...
and one shard per kind**, and the histories beside it are one shard per kind
and century since I5; see the index tree above.
...
│   ├── imports/naturalearth-places.json  ● import-places: which Natural Earth city a place record is, keyed by NE_ID. WRITTEN by tools/import/naturalearth.mjs --places for the matches it can prove, and by a person for the rest, from the candidates it lists in docs/naturalearth-places.md; a hand-written entry survives the next run
...
│   ├── geo/
│   │   ├── LICENSE               ● per-source: Natural Earth 110m and 10m = public domain, CShapes = CC BY-NC-SA 4.0
│   │   ├── land-present.json     ● the coastline at the far level: Natural Earth **10m**, simplified to fit 200 KB, GENERATED by tools/import/naturalearth.mjs since M36a (110m until then). The only base-map bytes at first paint
│   │   ├── base/<layer>/<cell>.json  ● the base map at the near level, GENERATED by tools/import/naturalearth.mjs: one file per non-empty cell of the fixed 60x45 degree grid of src/map/grid.js, keyed x0y0..x5y3, fetched when the viewport enters the cell and never at first paint. `coast` is lines (the shore inside the cell, so no cut ring is stroked as a ring), rivers and lakes and physical regions and mountains from M36b, cities from M36c, `relief` — the five elevation bands — from M45b: seven layers, all of them there. See **The base map** in the data model
...
│   │   └── presences/<from>-<to>.json  ● outlines, sharded by period, GENERATED by tools/import/cshapes.mjs, with `arcs` beside `features`: the inland borders between those territories, each held once and named by index from the features on either side (M39b)
...
│       └── history-<kind>-<key>-<hash>.json  ● what changed at each version of each record and when each signature was added, out of the repository's own commits, keyed by record id inside. One file per kind and century, filed by the same attributePeriod() the attribute shards use; fetched when a reviewer opens a record, and one file serves every record of that kind and century (I5). One file per record until then — 1,027 of them and 4.1 MB on disk, 62,446 at 10^4, all committed and all shipped
│
├── schema/
...
category and the layer control toggles them — `src/map/glyphs.js` holds one
`<symbol>` per category in a `<defs>` the document carries once, the map draws
a `<use class="glyph">` over each mark and the timeline one at the left of each
bar wide enough for it, and the toggles, inside a collapsed `<details>` and
each carrying its own symbol, are the map's whole legend for categories. An
event with no category keeps the plain circle, and the filter is one removal in
`workingSet` (`src/emphasis.js`), so the map, the timeline, the graph and the
corner count narrow together. **`category` is a core column**, not an attribute
one: a toggle that hides marks must hide them on the frame it is clicked, and
an attribute column arrives with its century. The Wikidata class table
...
### The base map ● — the ground everything else is read against

`data/geo/base/`, written by `tools/import/naturalearth.mjs` from the Natural
Earth 10 m files committed under `vendor/`, gzipped, with their sha256 in
`vendor/SHA256SUMS`. The tool is offline and idempotent: it has no network at
all — a `fetch` in it is a bug and not a fallback — and a second run over the
same inputs rewrites every file byte for byte, so a regenerated base map is a
diff of what actually changed.

**Seven layers**: `relief`, `coast`, `rivers`, `lakes`, `physical` (deserts,
plateaux, ranges and the other landforms, by a frozen allow-list of
`FEATURECLA`), `mountains` (the elevation points, each with the height the
file gives it) and `cities`. The first five are geometry; the last two are
...
anything else has arrived; the **near** level is the same layer at finer
detail, cut on the grid. `coast` has no far file of its own because its far
level *is* `manifest.land` — `data/geo/land-present.json`, which
`loadAtlas` already fetches at first paint and which keeps the name, the
shape and the manifest key the 110 m coastline had.

**The grid** is `src/map/grid.js`: a fixed 60° × 45° grid, six columns by
four rows, keys `x0y0`…`x5y3`, origin −180° **in data longitudes and not at
the seam**, so moving the central meridian renames no file. `cellsFor(box)`
is the question a viewport asks, and it wraps a box that crosses the
antimeridian. There is **one grid at one resolution** and the box on screen
decides which cell is fetched, never the zoom: that is what keeps this a
directory of our own files rather than a tile scheme with another name (see
the no-tiles boundary under Decisions taken). **A cell with nothing in it is
not written and is not in the manifest.**

**What a cell holds is decided per layer**, and one sentence decides it: *a
cut edge is never part of a stroked ring.* Lines — the coast and the rivers —
are **clipped** to the cell, because a stroke cut at a cell edge is the same
stroke. Polygons — the lakes and the physical regions — arrive **whole** in
every cell their bounding box overlaps, carrying Natural Earth's own id so
that M37 draws each once however many cells brought it; a clipped fill's cut
edge would be a hairline along a cell border, a shore that does not exist.
Points are simply in the cell they fall in.

**Simplification is by cap, not by taste.** Each level's tolerance is
whatever its byte cap forces, found by stepping a ladder until the output
fits, and `--budget` prints per layer the tolerance, the points kept and the
points dropped before a byte is written. The tool **refuses to write at all**
if the base map would pass 8 MB or `data/geo/` 24 MB. Where the bytes are
decided by the feature *count* rather than by the tolerance — which is every
far level, a ring never being fewer than four points — the layer also has a
floor: a ring under 0.007 square degrees, a river under 1.9 degrees of
length, a lake under 0.05 square degrees, a city under 250,000 people.
Nothing is floored in a cell: a cell is where the small things are, and a
reader who fetched one asked for them. Everything any floor drops is written
down, in `STATUS.md` and in `data/geo/LICENSE`, and is never a silent trim.

**The cities are the one layer that is filtered rather than simplified**,
a point having nothing along it to take off. What is kept is every populated
place over 100,000 **plus every populated place a `data/places/` record
names, whatever its population** — the second half being
`data/imports/naturalearth-places.json`. Each carries `id`, `name`, `nameEn`
where it differs, `lon`, `lat`, `pop`, `z`, `zl`, `wikidata` and, where the
mapping names one, `place`.

**Zoom is in the data and in one unit.** Every feature carries `z`, the `k`
from which it is worth drawing, and a city also carries `zl`, the `k` from
which its label is; each layer carries a `minZoom` in the manifest. `k` is
`src/map/projection.js`'s — `k = 1` is the whole world in 960 units — and
Natural Earth's own tile zoom is converted into it by **one frozen, monotone
table**, gentler than the arithmetic on purpose, with everything on the map
by `k = 16`. So **simplification at run time is a matter of dropping features
by their own threshold** and not of a second simplifier in the browser: a
view at `k` draws the features whose `z` is at or under it, out of whichever
level it has in hand, and `src/util/simplify.js` is still there for an
outline that needs taking down further. No zoom threshold is in code, in
either half.

**The manifest's `base` block** names the source, the version, the grid and,
per layer, its `geometry` (`polygon | line | point`, which is what M37
dispatches on), its `world` file or `null`, its `minZoom` and its cells with
a `bytes` each — so a reader decides what to fetch without a HEAD request and
a reviewer can see what the map costs without walking a directory. It is
built by scanning what is on disk: **the builder never invents a file name
and never names a file that is not there**, because a manifest entry that
404s is a layer which silently does not draw. A dataset with no
`data/geo/base/` gets **no `base` key at all** — absent, not empty, exactly
as an absent `presences` says there are none.
...
manifest its `base` block; 7 since the glyph run, which moved `category` into
the core; 6 since I5, which took the histories into hashed files; 5 since I4b,
which stopped writing the whole-corpus file; 4 since I3, 3 since I2, 2 since
I1 — and the
...
| Kind | And |
|---|---|
| event | `title`, `revised`, `when` (verbatim), `place`, `region`, `weight`, `actors` as `[{ actor, role, note? }]`, `citesCount`; and where the record has them, `parent`, `scope`, `category` and `subtreeWeight` (`category` is a core column since the glyph run, the rest of this row's optional three as listed) |
...
**The histories are hashed files like everything else since I5**, and there is
no unhashed thing left in the index. They were `history/<id>.json`, one file
per record under a directory the manifest named as a directory: not hashed,
because the dashboard fetched a history by the record's id and a hashed name
would have meant reading the manifest for each one. That was 1,027 files and
4.1 MB of disk on the real data and 62,446 files at 10^4, every one of them
committed on `main` and shipped in the artifact so that a reviewer could see
the versions of the one record in front of them (index2-plan, D8).

`history-<kind>-<key>-<hash>.json` is one kind and one century, keyed by
record id inside, named in the manifest under `historyShards` in kind order
and then year order. `key` is what `attributePeriod` says the record's period
is — the same table the attribute shards file by, so the two schemes cannot
come to disagree about which file a record is in — with a place and a source
filed by their kind, since neither has a year, and anything else with no year
in the `null` shard. The dashboard works that key out from the core, which
carries the year bounds the table reads, and holds the shard: a reviewer
working through one kind in one century fetches one file for the session.
With the directory went the two directory special cases in `readIndex` and
`writeIndex`, which are the citer directory's alone again.

**Rule 16 compares the histories byte for byte**, which it has since H9 and
which the sharding does not loosen. It could not before: a history is derived
from the repository's commits and not from `data/` alone, and the deploy
checked out one commit deep, so the two builds rule 16 put side by side did
not have the same commits to read. What makes the comparison hold is that a
version is identified by the record's own content — its `revised` and the
fields that changed — and never by the commit that carried it, so a build made
before a change is committed agrees with one made after; and that a shallow
clone is refused outright and falls back to `revised` for the whole tree
rather than claiming `from: "git"` over a horizon (`tools/lib/history.mjs`,
`isShallow`).
...
six — `relief`, `rivers`, `lakes`, `physical`, `mountains`, `cities` — with a category of
...
| `history-<kind>-<key>-<hash>.json`, 18 files | 545,495 | | no — one file, when a record is opened, and one for every record of its kind and century |
...
**And what M36a did to it**, measured on the corpus of 15 September 2026 (421
events, 175 of them categorised) after the glyph run: the coastline is 10 m
now and the whole first paint is **329,952 B (322.2 KB)** — `manifest.json`
33,895, the core 69,859, the sources 33,681, `land-present.json` **188,446**
against the 126,436 it was, and the palette 4,071. The coastline is still the
largest item and still the only base-map bytes before the first picture: the
2,392.5 KB of `data/geo/base/coast/` is fetched a cell at a time as the
viewport enters one, and `tests/spine-pages.test.mjs` holds every page to
asking for none of it before it draws. Decision 9's "first view under 1 MB"
holds with two thirds to spare, and what the extra 62 KB bought is Malta,
Bahrain, Madeira, Santa Maria and Graciosa, Santiago, Barbados and Bermuda —
islands the 110 m coastline does not have.

**And what the whole of M36 came to**, on the same corpus: the base map is
**6,175,234 bytes (6,030.5 KB) of its 8 MB ceiling** in 135 files — the
coastline's twenty-four cells 2,449,884, the rivers' nineteen 1,171,172 and
their world file 201,218, the lakes' twenty 583,508 and 138,012, the physical
regions' twenty-three 706,306 and 242,858, the peaks' twenty-four 69,795 and
69,749, the cities' twenty 348,242 and 194,490 — and `data/geo/` in total is
**11,478,981 bytes (10.9 MB) of its 24 MB**, against 5.0 MB before M36a. Not
one of those bytes is fetched before a picture. What first paint actually
grew by is the manifest, which names all 130 cells with their bytes so that
M37 need never send a HEAD: 33,895 → **48,351**, and the whole first paint
**344,408 B (336.3 KB)** — the manifest, the core 69,859, the sources 33,681,
`land-present.json` 188,446 and the palette 4,071. Decision 9's "first view
under 1 MB" holds with two thirds to spare.
...
is not in the artifact at all: `manifest.schema` is 8, `writeIndex` removes the
...
- **No map library, no tile server, no API key.** The map is our own SVG over
  our own projection (`src/map/projection.js`), and the geometry under
  `data/geo/` is our own vector files, from our own origin, generated offline
  by a Node script from a public-domain source and committed. That is the
  **boundary** of the no-tiles rule and not an exception to it (owner,
  5 September 2026): what the rule forbids is a third party's server deciding
  what the reader sees, a key that can be revoked, a bill that can arrive, and
  modern borders on a historical map. A directory of files this repository
  wrote, serves from its own origin and can regenerate offline is none of
  those, whether it is one coastline or the twenty-four cells of M36's base
  map. Nothing in it is a zoom pyramid, either: there is one grid at one
  resolution and the viewport decides which cell is fetched, never the zoom.
