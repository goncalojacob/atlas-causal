...
│       ├── manifest.json         ● schema version, counts, hashed file names, roles in use, land epochs; never cached
│       ├── topology-<hash>.json  ● every event {id,title,when,where,region,status,actors,weight}, every edge {id,from,to,type,confidence,status}, every actor {id,actorType,name,names,when,status}
│       └── sources-<hash>.json   ● every source record; small, needed for citations
│
├── schema/
│   ├── v1/event.json  edge.json  source.json  actor.json  region.json  bundle.json  ●
...
│   └── new-record.mjs            ● scaffold a record locally, of any of the four kinds
│
...
Reserved, by this line only ○: `data/presences/`, `data/narratives/`, `data/i18n/<lang>/`, `data/geo/land-<epoch>.json`,
...
edges, sources, actors, regions), the hashed file names of the topology and
sources indexes (served `immutable`), the set of `roles` in use, and land
...
- **Presence** — `kind: presence`, `actor`, `presenceType`
  ...
  `geometry` ref, confidence, sources; `actor` points at an actor record,
  which now exists. `presenceType` drives rendering:
...
Rules 6, 10 and 11 reach actors: an actor cites at least one source; its
`where`, if present, is within WGS84, but it needs no `region`, having no
lane; and a merged or retracted actor may not be referenced by an active
event.

Warnings: arrow of time fails the strict bound (`from.start.max ≤
to.start.min`); an event with degree zero; a source with no citers; an
actor no event references; an event dated entirely outside an actor's
dates — a warning and not an error, because posthumous events are real and
institutions act through their successors.
