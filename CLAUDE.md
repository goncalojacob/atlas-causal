...
data/categories.json       the twelve kinds of event; a closed vocabulary in data, offered by the form and the review editor, drawn as a glyph per category from M32b
...
the note beside a role as a third column on the actor row, and the role
itself offered from `data/roles.json` and not held to it, because the check
is still a warning.
