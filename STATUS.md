...
2026-09-05, after H3a-1 (`docs/health/h3a-brief.md`, amendments A0–A14), the
...
nothing under `data/` moved, `data/index/` was regenerated by this run, which
owns it. 706 tests.
...
Nothing reads any of it yet. H3a-2 adds `loadSpine()` and
`createAtlasFromSpine()` with the round-trip suites; H3b moves the pages over
one commit each. `src/data.js` is untouched.
...
18. **Next build: H3a-2** (`docs/health/h3a-brief.md`, amendment A0) —
   `loadSpine()`, `createAtlasFromSpine()` yielding the atlas surface A11
   lists, and the round trip proved by parameterising the card, horizon,
   entry and graph suites to run twice, once over each atlas (A12). Nothing
   under `data/` changes and `data/index/` is not rebuilt. Then H3b, where
   the pages switch over one commit each and the citer rows finally leave the
   sources index (deviation 216). H5b may run beside it on its own branch,
   without committing `data/index/`.
   H3a-1 is done: the emitter, the search shard, the citer directory, the
   file machinery and the measured sizes above. Two things it leaves H3b
   rather than H3a-2, because both are loader work: the strip, and
   `retractionPlan` taking a pre-fetched citers map — the test that it
   answers the same off one citer file as off the whole index is already
   written, in `tests/citers.test.mjs`.
