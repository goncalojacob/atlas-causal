...
src/layer-control.js       the layer switches and, inside a collapsed `<details>`, one toggle per category in use, each carrying its glyph — the map's whole legend for categories. Built rather than written into index.html because a label comes from `data/`; writes M30b A11's token set, the bare `events` while every category is on and one `events:<id>` per category still on when one is not
