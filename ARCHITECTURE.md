...
│       ├── manifest.json         ● schema version, counts, hashed file names, lanes, roles in use, the two vocabularies (rolesAllowed, categoriesAllowed), officesByEvent and tenuresByOffice, land epochs, presence shards, explanation shards, the palette's file name; never cached
│       ├── spine-<hash>.json     ● the graph, loaded whole by every page: every event {id,title,when,place,region,status,actors,weight,citesCount} with parent, scope, category and subtreeWeight where it has them, every place {id,name,names,where,region,status}, every edge as [from,to,type,confidence,status,revised], every actor {id,actorType,name,names,when,status}, every relation {id,from,to,type,when,note,status}, every office {id,of,title,category,when,status} with every tenure {id,person,office,when,startedBy,status}, every narrative {id,title,summary,authors,window,steps:[{ref}],status} — the refs and not a word of the prose, every presence {id,actor,dependencyOf,dependencyKind,when,geometry.key,capital,confidence,status}
...
ranges. Beside `regions` it carries the two closed vocabularies that live in
...
the order they were held. Both are derived from records the spine already
carries whole; they are here so that `lanes.js` is a lookup in M33 rather than
a scan of every tenure per event. The spine carries every presence **without its coordinates**, so
...
These files are the whole of the index a page reads. H3a-1 emitted them
