...
230. **The band moves the nodes now, which H3b said it must not.**
...
231. **The brief says code only and two sentences of `ARCHITECTURE.md`
...
232. **What the reader is holding enters the arrangement key as six state
...
233. **The Worker exists, because the number asked for it.** The brief
...
234. **`crosses` is exported.** The test that holds the swept count to the
     ...
     testing itself.
235. **The stacking is filed under the layout's key, not the arrangement's
