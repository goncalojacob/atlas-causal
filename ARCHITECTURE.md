...
│   │   ├── base/<layer>/<cell>.json  ● the base map at the near level, GENERATED by tools/import/naturalearth.mjs: one file per non-empty cell of the fixed 60x45 degree grid of src/map/grid.js, keyed x0y0..x5y3, fetched when the viewport enters the cell and never at first paint. `coast` is lines (the shore inside the cell, so no cut ring is stroked as a ring) since M36a; rivers, lakes, physical regions, mountains and cities follow in M36b and M36c
