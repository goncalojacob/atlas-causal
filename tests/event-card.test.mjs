...
async function atlasFrom(dir) {
  const read = async (rel) => JSON.parse(await readFile(path.join(dir, rel), 'utf8'));
...
const atlas = await atlasFrom(path.join(ROOT, 'data'));
const fixtures = await atlasFrom(FIXTURE_DATA);
...
// nothing fetched.
test('the sections count what the topology and the sources index actually hold', () => {
  const html = eventCardHtml(context(atlas), { event: carnation, found, state: state() });
