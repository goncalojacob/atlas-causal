...
data/categories.json       the twelve kinds of event; a closed vocabulary in data, offered by the form and the review editor, drawn as a glyph per category from M32b-2
...
data/index/manifest.json   GENERATED, never cached: the index's generation, counts, the hashed names below, lanes and one box per lane, roles in use, the two vocabularies, officesByEvent and tenuresByOffice, land epochs, presence shards, attribute shards
data/index/spine-<hash>.json      GENERATED: the graph, loaded whole by every page — every record as a positional row over the file's own id table, with the closed vocabularies as integers and the columns named in the file; an event's title, when, place, lane, weight and actors, an edge still [from,to,type,confidence,status,revised], and not a word of the prose
...
data/index/core-<hash>.json       GENERATED, read by no page yet: the spine split in two by I3 — the graph and what a mark, a bar and a lane are drawn from (ids, statuses, an event's year bounds, place, lane, weight and its actors' ids, an edge's five slots, an actor's type and years, a place's point, the ids each relation, office and tenure joins)
data/index/attributes-<key>-<hash>.json  GENERATED, read by no page yet: the other half — title, when verbatim, revised, citesCount, identity, scope, category, roles and notes, names, a place's label and precision. One file per century, plus `place` and `null`; I4 is what moves the pages over, if the bytes say the split pays
...
src/main.js                bootstrap only: load, wire views, build the layer switches; ?fixtures=1 reads tests/fixtures/data/
...
src/state.js               { from, to, view, focus, group, lanes, selected, source, place, actor, office, chain, horizon, layers, narrative, step, bbox } ⇄ URL; a null bound is "as far as the data goes", and `layers` takes a category of events as `events:<id>` beside the three names
...
src/data.js                manifest → the spine (whole), or since I3 the core and its attribute shards, which no page reads yet → record text on demand; aliases, adjacency by edge and by relation, events by actor and by place, an event's point through its place, one geometry shard per year
